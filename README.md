...
1.4.98 将样式治理策略的内部结果统一为 `violations`，仅在 Stylelint Gate 适配其原生结果协议时转换为 `warnings`，避免继续用 warning 名称表达 `severity: error` 的治理违规。Stylelint 的真实 warning 阈值和原生 `warnings` 字段保持不变；样式治理行为、配置、Schema、GateResult、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.97

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.97
npx repo-guard doctor
```

1.4.97 删除 pre-commit 最终策略 runner 对依赖策略与保护文件 Gate 的重复硬编码分发，改为直接使用统一 orchestrator 的 Gate `plan`/`run` 执行。暂存质量与保护文件仍是独立模块，执行顺序仍由受保护 Execution Plan 固定；lint-staged 隔离、部分暂存恢复、配置、GateResult、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.96

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.96
npx repo-guard doctor
```

1.4.96 删除 CI runner 对官方与外部 Gate 的具体分发 switch，改为统一使用 orchestrator 的 Gate `plan`/`run` 执行。既有 CI 步骤显示名由受审 Execution Plan 的 `reportName` 保存，Stylelint、ESLint 与 Prettier 各自在 Gate plan 中按项目 pattern 选择适用文件；CI runner 只负责范围、上下文、报告聚合与持久化。报告名称、步骤顺序、文件范围、GateResult、profile、Hook、公共 exports 和退出码均不变。
...
1.4.93 将原先混在 `repository/native-policy-gates.js` 的 Vue security、Vue accessibility 与 repository Gate 拆回所属领域，并由 Registry 按原顺序组合；共享的 `gates/vue-policy-gate.js` 只负责 Vue policy Gate 协议，不知道具体规则。旧聚合路径已删除且不保留兼容转发，capability ID、环境、顺序、超时、结果、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.92

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.92
npx repo-guard doctor
```

1.4.92 用 error 级 dependency-cruiser 规则固定 config、Git 与 policy 基础领域的单向依赖骨架：config 不得反向依赖 Git、policy 或运行层，Git 不得依赖 policy 或运行层，policy 可以消费 integration 提供的无决策事实但不得依赖 Gate 或 orchestration。独立违规 fixture 会验证每条规则确实生效。该版本只加强仓库自身的架构防回归，不改变消费项目配置、GateResult、Hook、CI、公共 exports 或退出码。

## 升级到 1.4.91

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.91
npx repo-guard doctor
```

1.4.91 将 Git 仓库状态持久化、暂存指纹与 Git index 元数据读取统一归入 `src/git`，删除 `integrations/git` 且不保留兼容转发。状态文件、指纹算法、暂存元数据、错误契约、GateResult、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.90

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.90
npx repo-guard doctor
```

1.4.90 将结构化例外的日期状态计算与有效期断言归入 `config/exception-lifecycle.js`，配置加载、迁移、doctor、pre-push 与 Gate 直接消费该配置生命周期；`policies/exception-registry.js` 只负责 finding 与当前有效例外的精确匹配。旧内部导出已删除且不保留兼容转发，配置行为、错误代码、GateResult、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.89

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.89
npx repo-guard doctor
```

1.4.89 移除 dependency-cruiser 中已无目标的 `commands` 过渡分支，并从受审源码目录清单移除空目录要求。全新 clone 与已有工作区现在使用同一目录契约；现有 core、gate、integration、orchestration 依赖方向、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.88

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.88
npx repo-guard doctor
```

1.4.88 删除 `src/commands/doctor.js`，将只读诊断、CI 模式检查、Gate setup 巡检与结果输出归入 `src/orchestration/doctor/runner.js`，不保留旧路径兼容转发。Node 契约和仓库修复继续保持独立，过渡 `commands` 层已无源文件；诊断行为、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.87

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.87
npx repo-guard doctor --fix
```

1.4.87 将 `doctor --fix` 的配置创建/迁移、受管策略维护和 Hook 重装写操作拆入 `src/orchestration/setup/repository-repair.js`；doctor 通过结构化修复结果继续诊断。只读 doctor、修复范围、安全约束、输出、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.86

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.86
npx repo-guard doctor
```

1.4.86 将最低 Node.js 版本契约、版本解析和支持判断从 doctor 编排拆入 `src/core/project/node-version.js`。doctor 只消费项目运行时事实；Node.js `>=22.23.2` 要求、诊断结果、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.85

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.85
npx repo-guard init
```

1.4.85 删除 `src/commands/init.js`，将能力探测、配置创建、受管 Hook 安装、策略维护与初始化结果输出归入 `src/orchestration/setup/project-initialization.js`，不保留旧路径兼容转发。`install-hooks` CLI 适配继续保持独立；初始化行为、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.84

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.84
npx repo-guard init
```

1.4.84 将 `install-hooks` 的 CLI 适配、可选 Git 仓库处理和结果输出从项目初始化入口拆入 `src/orchestration/cli/install-hooks.js`；CLI runner 直接调用该模块。受管 Hook 内容与升级兼容、项目初始化、配置、CI、公共 exports 和退出码均不变。

## 升级到 1.4.83

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.83
npx repo-guard init
```

1.4.83 删除 `src/commands/pre-push.js`，将推送范围收集、GateContext 构造和固定 Execution Plan 编排归入 `src/orchestration/pre-push/runner.js`，不保留旧路径兼容转发。推送配置与精确快照解析继续保持独立；门禁顺序、跳过语义、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.82

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.82
npx repo-guard init
```

1.4.82 将推送修订配置加载、门禁启用判断及精确 HEAD/干净工作区快照校验从 pre-push 生命周期入口拆入 `src/orchestration/pre-push/push-configuration.js`。pre-push runner 只消费解析结果并编排门禁；推送范围、跳过语义、配置错误、门禁顺序、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.81

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.81
npx repo-guard init
```

1.4.81 删除 `src/commands/pre-commit.js`，将 pre-commit 生命周期、暂存配置读取和保护策略执行归入 `src/orchestration/pre-commit/runner.js`，不保留旧路径兼容转发。`quality-files` CLI 适配继续由独立模块承载，固定质量检查顺序、lint-staged 暂存隔离、保护文件门禁、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.80

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.80
npx repo-guard init
```

1.4.80 将 `quality-files` 的参数接线、配置加载、质量执行调用与退出码转换从 pre-commit 生命周期入口拆入 `src/orchestration/pre-commit/quality-command.js`；CLI runner 直接调用该单一职责模块。pre-commit 的固定执行顺序、lint-staged 暂存隔离、保护文件门禁、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.79

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.79
npx repo-guard doctor
```

1.4.79 删除 `src/commands/ci.js`，将 CI 命令边界、配置与报告路径校验、失败报告回退和退出码转换归入 `src/orchestration/ci/command.js`；CLI runner 直接调用所属 CI 编排模块，不保留旧路径兼容转发。CI runner 与报告持久化继续保持独立，profile、变更范围、错误报告、报告路径安全、控制台输出、配置、Hook、公共 exports 和退出码均不变。
...
1.4.77 删除 `src/commands/hook-message.js`，将 `prepare-commit-msg`、`commit-msg` 与 `post-commit` 共用的提交信息 Hook 生命周期编排归入 `src/orchestration/commit-message/runner.js`；CLI runner 只负责分发，不保留旧路径兼容转发。模式校验、仓库与配置加载、prepare/finalize/cleanup 调用顺序、提交信息摘要策略、受管 Hook 内容与版本、配置、CI、公共 exports、输出和退出码均不变。

## 升级到 1.4.76

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.76
npx repo-guard doctor
```

1.4.76 删除 `src/commands/install-ci.js`，将只服务 CLI 的 GitLab CI 安装参数接线与结果展示归入 `src/orchestration/cli/install-ci.js`；CLI runner 直接调用同层模块，不保留旧路径兼容转发。provider 校验、仓库发现、安装与 dry-run 行为、复杂配置的手动回退提示、控制台输出、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.75

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.75
npx repo-guard doctor
```

1.4.75 删除 `src/commands/configure.js`，将只服务 CLI 的配置迁移、功能启用和功能禁用编排归入 `src/orchestration/cli/configuration.js`；CLI runner 直接调用同层模块，不保留旧路径兼容转发。配置迁移、功能开关、架构/可访问性/单元测试 AI 策略同步、控制台输出、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.74

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.74
npx repo-guard doctor
```

1.4.74 删除 `src/commands/check.js`，将仅服务手动 CLI 的受保护工作区检查归入 `src/orchestration/cli/check.js`；CLI runner 直接调用所属模块，不保留旧路径兼容转发。配置加载、本地环境暂存泄露阻断、工作区变更收集、保护规则分类、finding、控制台输出、配置、Hook、CI、公共 exports 和退出码均不变。

## 升级到 1.4.73

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.73
npx repo-guard doctor
```

1.4.73 删除根级 `src/cli.js`，将 CLI 帮助生成、命令分发、运行期参数接线与统一错误呈现归入 `src/orchestration/cli/runner.js`；唯一 npm `bin` 启动器直接调用该编排模块，不保留旧路径兼容转发。命令顺序、帮助文本、参数语义、错误分类与呈现、配置、Hook、CI、公共 exports、输出和退出码均不变。`src` 根目录现在只保留 npm 公共入口 `index.js`。
...
```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.70
npx repo-guard doctor
```

1.4.70 将 CLI 无值选项校验和带值选项解析迁入 `src/orchestration/cli/argument-parsing.js`，`src/cli.js` 继续只在命令分发点声明各命令支持的参数。支持项、未知选项、缺失值、重复值覆盖及错误文本保持不变；命令分发、帮助文本、错误呈现、配置、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.69

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.69
npx repo-guard doctor
```

1.4.69 将各配置领域的验证与规范化编排迁入 `src/config/configuration-validation.js`，`src/config.js` 只保留公共导出、统一错误包装和配置文件加载。领域调用顺序、默认值、规范化结果与错误优先级保持不变；根契约、保护文件和 staged-code 质量配置仍由各自模块负责，schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.68

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.68
npx repo-guard doctor
```

1.4.68 将根配置对象、顶层已知字段及配置版本校验迁入 `src/config/root-configuration-validation.js`，主配置验证器在原时序点调用根契约后继续编排各领域结果。根对象要求、允许字段集合、版本 1 契约及错误顺序保持不变；各领域配置、保护文件、staged-code 质量配置、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.67

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.67
npx repo-guard doctor
```

1.4.67 将 `preCommit` 容器形状验证及 file-placement、max-file-lines、Stylelint、Prettier、ESLint 五类 staged-code 质量配置的编排迁入 `src/config/pre-commit-validation.js`，主配置验证器只接收规范化后的 `preCommit`。容器对象与已知字段校验、五类子配置验证顺序、默认值和返回结构保持不变；保护文件规则仍由独立模块负责，Hook 执行顺序、加载、schema、CLI、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.66

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.66
npx repo-guard doctor
```

1.4.66 将项目通知开关配置验证迁入 `src/config/notification-validation.js`，由领域模块返回规范化后的 `notification`，主配置验证器仅编排结果。通知对象、已知字段、布尔开关及默认值保持不变；通知文本策略、企业微信网络集成、保护文件、staged-code 质量配置、其他门禁、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.65

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.65
npx repo-guard doctor
```

1.4.65 将根级保护文件 `rules`/`exclusions` 的集合预检、规则字段验证、路径清洗与 matcher 编译迁入 `src/config/protected-file-validation.js`，与 pre-commit staged-code 质量配置保持独立；主配置验证器仍在原时序点执行形状预检和规范化。至少一条规则、排除项数组、pattern/category/level、`notify`/`audit` 级别、首条规则匹配、排除优先级、Git 路径规范化、错误顺序及 `SUPPORTED_LEVELS` 兼容导出保持不变；其他门禁、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.64

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.64
npx repo-guard doctor
```

1.4.64 将 pre-commit ESLint 暂存代码质量配置验证迁入 `src/config/eslint-validation.js`，由领域模块返回规范化后的 `eslint`，主配置验证器仅编排结果。对象与开关校验、托管 preset、文件模式、自动修复、最大警告数、字符串清洗及默认值保持不变；file-placement、max-file-lines、Stylelint、Prettier、其他门禁、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.63

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.63
npx repo-guard doctor
```

1.4.63 将 pre-commit Prettier 暂存格式化配置验证迁入 `src/config/prettier-validation.js`，由领域模块返回规范化后的 `prettier`，主配置验证器仅编排结果。对象与开关校验、文件模式、自动修复、配置文件要求、字符串清洗及默认值保持不变；file-placement、max-file-lines、Stylelint、ESLint、其他门禁、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.62

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.62
npx repo-guard doctor
```

1.4.62 将 pre-commit Stylelint 顶层执行参数、选择器/嵌套复杂度与样式治理配置验证迁入 `src/config/stylelint-validation.js`，由内聚领域模块返回规范化后的 `stylelint`，主配置验证器仅编排结果。对象与开关校验、文件模式、自动修复、最大警告数、配置要求、复杂度阈值、specificity/ID/important 治理、全局样式路径、父子启用依赖、字符串清洗及默认值保持不变；file-placement、max-file-lines、Prettier、ESLint、其他门禁、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.61

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.61
npx repo-guard doctor
```

1.4.61 将 pre-commit 单文件行数配置验证迁入 `src/config/max-file-lines-validation.js`，由领域模块返回规范化后的 `maxFileLines`，主配置验证器仅编排结果。对象与开关校验、严格/不回退模式、预警比例、文件匹配规则、最大行数、排除路径、字符串清洗及默认值保持不变；file-placement、Stylelint、Prettier、ESLint、其他门禁、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.60

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.60
npx repo-guard doctor
```

1.4.60 将 pre-commit 文件归位配置验证迁入 `src/config/file-placement-validation.js`，由领域模块返回规范化后的 `filePlacement`，主配置验证器仅编排结果。对象与开关校验、检查模式、规则结构、匹配/允许/例外路径、建议目录约束、字符串清洗及默认值保持不变；max-file-lines、Stylelint、Prettier、ESLint、其他门禁、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.59

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.59
npx repo-guard doctor
```

1.4.59 将 Vitest 单元测试、覆盖率、组件交互、文件匹配与测试映射配置验证迁入 `src/config/unit-test-validation.js`，由内聚领域模块返回规范化后的 `unitTest`，主配置验证器仅编排结果。对象与开关校验、npm 脚本名、超时、覆盖率目录与阈值、组件交互依赖、文件模式、映射占位符、字符串清洗及默认值保持不变；pre-commit、其他门禁、通知、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.58

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.58
npx repo-guard doctor
```

1.4.58 将 axe 可访问性测试配置验证迁入 `src/config/accessibility-validation.js`，由领域模块返回规范化后的 `accessibilityTest`，主配置验证器仅编排结果。对象与开关校验、npm 脚本名、超时、测试文件匹配模式、字符串清洗及默认值保持不变；unit-test、pre-commit、外部门禁、通知、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.57

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.57
npx repo-guard doctor
```

1.4.57 将 build、Vue Lighthouse 与 TypeScript 三类外部执行型质量门禁配置验证迁入 `src/config/execution-gate-validation.js`，由内聚模块返回规范化后的 `build`、`lighthouse` 与 `typeCheck`，主配置验证器仅编排结果。各门禁对象与开关校验、npm 脚本名、超时、Lighthouse 配置路径、可空构建脚本、字符串清洗及默认值保持不变；accessibility/unit-test、pre-commit、通知、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.56

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.56
npx repo-guard doctor
```

1.4.56 将依赖架构配置验证迁入 `src/config/architecture-validation.js`，由领域模块返回规范化后的 `architecture`，主配置验证器仅编排该结果。启用开关、超时、源路径、TypeScript 配置路径、排除正则、规则名称与严重级别、from/to 条件正则及条件深拷贝行为保持不变；构建、其他质量门禁、通知、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.55

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.55
npx repo-guard doctor
```

1.4.55 将依赖治理配置验证迁入 `src/config/dependency-policy-validation.js`，由领域模块返回规范化后的 `dependencyPolicy`，主配置验证器仅编排该结果。启用开关、精确版本与锁文件策略、协议名称校验和去重、禁用包唯一性、原因长度、替代包清洗及默认值保持不变；架构、构建、质量门禁、通知、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.54

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.54
npx repo-guard doctor
```

1.4.54 将结构化例外配置验证迁入 `src/config/exception-validation.js`，由领域模块返回规范化后的 `exceptions`，主配置验证器仅编排该结果。`warningDays`、`maxDays`、ID 与目标唯一性、命名空间规则、精确仓库路径、行列位置、理由与工单长度、owner/approvedBy 分离、ISO 日期和生命周期约束保持不变；依赖治理、架构、质量门禁、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.53

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.53
npx repo-guard doctor
```

1.4.53 将 CI 配置、受保护文件动作与外部项目门禁验证迁入 `src/config/ci-validation.js`，由领域模块返回规范化后的 `ci` 与 `externalGates`，主配置验证器仅编排该结果。CI 默认值、profile、报告路径、外部门禁字段、环境、脚本、超时、Windows 保留名防护、大小写不敏感的报告路径唯一性和数组复制行为保持不变；通知、例外、依赖、质量门禁、加载、schema、CLI、Hook、CI 输出和退出码不变。

## 升级到 1.4.52

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.52
npx repo-guard doctor
```

1.4.52 将配置文件名、结构化配置错误、已知字段检查、ISO 日期、仓库内相对模式、模式列表与 CI 报告路径校验迁入 `src/config/validation-primitives.js`，内部消费者直接依赖新模块，原配置入口继续显式重导出既有公开名称。错误代码与修复指引、日期有效性、路径越界防护、反斜杠规范化、空列表策略和 `reports/*.json` 约束保持不变；完整配置验证、默认值、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.51

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.51
npx repo-guard doctor
```

1.4.51 将 Git 路径规范化、glob 编译和配置规则匹配迁入 `src/config/path-matching.js`，内部消费者和公共入口直接依赖新模块，原配置入口继续显式重导出原有函数。反斜杠与 `./` 规范化、`*`、`**`、`?` glob 语义、正则字符转义、排除项优先级、首条规则匹配和返回结构保持不变；配置默认值、验证、加载、schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.50

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.50
npx repo-guard doctor
```

1.4.50 将 `src/config.js` 中独立的平台默认配置迁入 `src/config/defaults.js`，由原配置入口继续重导出全部常量。默认开关、脚本名、超时、glob、架构规则、覆盖率阈值、文件归位规则、冻结结构和现有导入契约保持不变；配置 schema、验证、加载、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.49

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.49
npx repo-guard doctor
```

1.4.49 删除混合职责的顶层 `src/git.js`：Git 子进程执行与可选值读取迁入 `src/git/execution.js`，仓库根目录发现与 Git 元数据路径解析迁入 `src/git/repository.js`，不保留兼容转发。`core.quotepath=false`、失败容忍与 fallback、类型化执行/配置错误、Windows 隐藏窗口、仓库发现和相对 Git 路径解析均保持不变；配置 schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.48

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.48
npx repo-guard doctor
```

1.4.48 删除混合职责的顶层 `src/git-changes.js`：Git `--name-status -z` 解析、revision/staged/working-tree 变更收集迁入跨入口共享的 `src/git/change-collection.js`，规则分类与展示路径迁入 `src/policies/change-classification.js`，不保留兼容转发。重命名/复制解析、类型化协议错误、变更状态合并、未跟踪文件收集、规则回退匹配与展示文本均保持不变；配置 schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.47

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.47
npx repo-guard doctor
```

1.4.47 删除顶层 `src/config-management.js`，将消费项目配置的初始化、迁移、功能启停与写回生命周期迁入职责更明确的 `src/orchestration/setup/config-management.js`，不保留兼容转发。starter 默认值、深拷贝隔离、迁移前验证、过期例外阻断、功能依赖联动、CI profile 配置和幂等写入行为均保持不变；配置 schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.46

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.46
npx repo-guard doctor
```

1.4.46 删除顶层 `src/exception-registry.js`，将结构化例外有效期分类、当前性断言与精确匹配策略迁入职责更明确的 `src/policies/exception-registry.js`，不保留兼容转发。active/expiring/expired/future 分类、UTC 日期边界、人工复审约束和规则/路径/行列精确匹配均保持不变；配置 schema、CLI、Hook、CI、公共 exports、输出和退出码不变。

## 升级到 1.4.45

```bash
npm install --save-dev --save-exact @cxyi7/repo-guard@1.4.45
npx repo-guard doctor
```

1.4.45 删除顶层 `src/vue-unsafe-html.js`，将 Vue `v-html` 安全规则、诊断与结构化例外应用迁入职责更明确的 `src/policies/vue-unsafe-html.js`，不保留兼容转发。仅分析 Vue template、忽略脚本字符串与注释、识别参数和修饰符、精确位置及结构化例外行为均保持不变；配置格式、CLI、Hook、CI、公共 exports、输出和退出码不变。
