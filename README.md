...
- 同一仓库只允许一个 pre-commit 生命周期操作 Git 索引、工作区和 `lint-staged` 备份；重叠提交会在任何修改前停止，异常退出遗留的生命周期锁可安全接管。
...
- 提供默认关闭的合同驱动交付门禁：用树形功能登记表确定功能归属，用 `schemaVersion: 2` 多文件合同包约束需求事实、Git 历史修订、目标分支、资料计划、Worktree/路径边界和执行清单，再由受 Git 跟踪的 Evidence Run 复核完整门禁结果、执行报告、人工验收、问题闭环与代码提交。启用功能会把五个标准项目级 Skill 安装到 `.agents/skills/`；资产模板默认不可误通过，技术证据形成、人工验收和最终 GateResult 比较采用两轮复核；禁用时只移除未被人工修改的托管文件。
...
- `release.delivery-evidence` 在其他发布门禁之后复核 Evidence Run 中的结构化证据与本轮完整 GateResult、目标分支漂移分析、人工确认、正式发现闭环、代码提交和内容指纹；任意说明文字不能代替可复算证据。
...
| [合同驱动交付格式](docs/contract-driven-delivery.md) | 功能登记表、Markdown 合同、需求快照、Git 历史修订、路径边界、反馈闭环与 Evidence Run 格式 |
