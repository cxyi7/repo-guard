...
项目向统一请求运行时、业务 API SDK、接口测试、页面业务测试和响应性能门禁演进时，采用一个 npm 包、多个受控子路径和四个能力平面的长期方案，详见[可扩展单包平台架构](docs/extensible-platform-architecture.md)。该文档描述目标结构，不代表所有规划能力已经实现。
