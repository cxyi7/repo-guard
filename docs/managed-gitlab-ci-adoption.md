...
本文记录 `front`、`owner`、`employee` 接入 `@cxyi7/repo-guard@1.8.0` 托管应用交付标准时的边界。它是迁移依据，不包含密钥、服务器地址、仓库凭据或其他环境秘密。
...
- GitLab 分支规则、Job 名、阶段、Runner 标签和发布通知触发点；
...
```json
{
  ...
  "ci:notify": "node scripts/notify-wecom-deploy-result.mjs"
}
```
...
1. 升级并精确锁定 `@cxyi7/repo-guard@1.8.0`。
...
迁移消费项目属于后续独立评审任务，不与 npm 包 `1.8.0` 的能力实现提交混在同一发布中。
