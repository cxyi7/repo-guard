...
2026-09-13 整合：本轮全部新增预设、检查与目录功能统一归入 `refactor/node-engineering-2.0.0`，目标版本为 `2.0.0`。原 2.1.0～7.2.0 为未发布的开发阶段编号。尚未完成的完整流程工作统一见[六项待办](end-to-end-workflow-backlog.md)；历史验收记录保留，不表示延期事项已经解决。
...
- **2026-09-14 已修复普通 CI 的受检版本绑定。** HEAD、索引、跟踪内容或配置与目标提交不一致时阻断；已补充未提交修复、执行期间切换提交及多应用回归，见 [T05 验收记录](end-to-end-workflow-backlog.md#t05修复普通-ci-的代码版本绑定)。部署闭环仍独立跟进。
- 两个固定真实消费项目的当前配置已完成 [CI 专项验收](ci-acceptance-2026-09-14.md)，包括实际安装产物、失败阻断、退出码和飞书通知；其他 JDK/系统及未覆盖组合仍待验证。Windows 超时样例曾出现清理目录 EBUSY，稳定性根因尚未独立定位，保留后续核验项。
