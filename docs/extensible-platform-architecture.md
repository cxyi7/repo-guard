...
| 过渡命令层 | 1.4.74 至 1.4.76 已迁移手动 `check`、配置开关和 GitLab CI 安装 CLI 适配，1.4.77 已迁移提交信息 Hook 生命周期编排；其余生命周期入口仍位于 `commands` | 每个入口只能随独立评审和独立版本迁入对应 `orchestration` 目录，不得批量移动或保留转发层 |
