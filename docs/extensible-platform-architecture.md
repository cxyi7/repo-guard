...
| 过渡命令层 | 1.4.74 至 1.4.85 已迁移通用 CLI、Hook 生命周期和项目初始化；1.4.86 至 1.4.87 已从 doctor 拆出 Node 契约与仓库修复，仅只读诊断入口仍位于 `commands` | 每个入口只能随独立评审和独立版本迁入对应 `orchestration` 目录，不得批量移动或保留转发层 |
