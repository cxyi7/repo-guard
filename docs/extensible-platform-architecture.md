...
| 过渡命令层 | 1.4.74 已迁移手动 `check` 编排，1.4.75 已迁移配置迁移与功能开关编排，1.4.76 已迁移 GitLab CI 安装 CLI 适配；其余生命周期入口仍位于 `commands` | 每个入口只能随独立评审和独立版本迁入对应 `orchestration` 目录，不得批量移动或保留转发层 |
