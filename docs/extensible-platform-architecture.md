...
| 不可变保护文件契约 | 1.6.1 在既有保护文件规则中增加 `block` 决策，不增加重复 Gate 或 Git 收集路径 | Git 层继续提供当前路径与原路径事实；repository gate 负责硬阻断、中文修复指引和阻断前禁止外部通知，`audit`/`notify` 语义不得改变 |
...
所有失败统一进入 `GateResult` JSON v2。规则违规返回 `kind: violation` finding；配置、执行、范围、安全、内部和取消异常由 `RepoGuardError` 分类。面向 AI 的 `issues` 必须自包含稳定代码、相对位置、中文问题说明、中文证据、中文预期、结构化中文修复步骤/约束/验证、审批要求和指纹。原始工具输出只能作为带来源、流、脱敏和截断元数据的 diagnostics，不能替代可操作的中文问题。机器 ID、命令、路径、包名、协议枚举和第三方规则 ID 不翻译；问题指纹只依赖稳定机器字段和位置，不依赖可本地化的展示文案。

中文要求由仓库指令、标准检查和架构测试共同守护。检查器已经完成 1.4.98 历史英文债务迁移，并以债务数为 0 的精确内容指纹基线执行全量零容忍；任何纯英文用户文案及其新增副本都会失败，禁止新增、替换、扩大或重新生成基线。不引入语言开关或通用 i18n 层。
...
- 每个门禁具有稳定 ID、执行环境、副作用、依赖和结果模型；`1.4.23` 已为全部 Gate 增加显式 `GateResult` 元数据，并冻结 24 个官方能力描述符的受审清单；
