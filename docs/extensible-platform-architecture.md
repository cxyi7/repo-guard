...
| 过渡命令层 | 1.4.74 至 1.4.77 已迁移手动检查、配置开关、GitLab CI 安装和提交信息 Hook 编排，1.4.78 已迁移手动暂存区保护门禁；其余生命周期入口仍位于 `commands` | 每个入口只能随独立评审和独立版本迁入对应 `orchestration` 目录，不得批量移动或保留转发层 |
