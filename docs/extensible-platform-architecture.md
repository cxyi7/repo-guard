...
| 过渡命令层 | 1.4.74 至 1.4.81 已迁移通用 CLI 与 pre-commit 入口；1.4.82 至 1.4.83 已分离推送配置解析并迁移 pre-push 生命周期，其余 setup 与 doctor 入口仍位于 `commands` | 每个入口只能随独立评审和独立版本迁入对应 `orchestration` 目录，不得批量移动或保留转发层 |
