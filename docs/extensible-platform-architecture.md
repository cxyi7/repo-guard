...
| 过渡命令层 | 1.4.74 至 1.4.83 已迁移通用 CLI、pre-commit 与 pre-push 入口；1.4.84 至 1.4.85 已分离 `install-hooks` 并迁移项目初始化，仅 doctor 入口仍位于 `commands` | 每个入口只能随独立评审和独立版本迁入对应 `orchestration` 目录，不得批量移动或保留转发层 |
