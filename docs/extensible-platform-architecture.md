...
| 过渡命令层 | 1.4.74 至 1.4.81 已迁移通用 CLI 与 pre-commit 入口；1.4.82 已从 pre-push 入口拆出推送配置和精确快照解析，其余生命周期入口仍位于 `commands` | 每个入口只能随独立评审和独立版本迁入对应 `orchestration` 目录，不得批量移动或保留转发层 |
