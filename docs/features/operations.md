...
没有运维配置时默认关闭，不会因为启用 ESLint、测试或构建而生成部署。当前支持 GitLab 生成器、Node 脚本部署，以及 Java Maven／Node 的[蓝绿部署](blue-green-deployment.md)。蓝绿入口也可手动执行；Java 使用独立原生构建配置，不会被当作 Node 应用。
