...
GitLab 新分支首次 push 的 `CI_COMMIT_BEFORE_SHA` 为全零时，若提供 `CI_DEFAULT_BRANCH`、当前分支不是默认分支且历史完整，工具从 `origin` 获取默认分支，并以它和目标提交的唯一共同祖先为基准，覆盖本分支的所有新增提交。设置 `GIT_DEPTH: "0"`；浅克隆、缺少必要平台变量、默认分支自身首次推送或没有唯一共同祖先时仍阻断。获取远程失败保留执行错误，不使用旧缓存替代。普通 push、合并请求和显式 base 保持原有优先级；无效显式基准不自动降级。
...
通知在 GitLab 环境优先读取合并请求源分支 `CI_MERGE_REQUEST_SOURCE_BRANCH_NAME`，然后区分标签 `CI_COMMIT_TAG`、普通分支 `CI_COMMIT_BRANCH` 和回退引用 `CI_COMMIT_REF_NAME`。本地执行读取实际 Git 分支；缺少平台信息且未附着分支时显示“指定提交检出（未附着分支）”，不显示“未命名分支”。这些字段仅用于通知展示，受检对象仍以目标提交和原有可信范围验证为准，不新增配置项。
