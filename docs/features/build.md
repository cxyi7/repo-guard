...
构建成功后，若启用 `build.artifactBudget`，继续检查[单平台产物预算](build-artifact-budget.md)。构建失败、工具无法启动或超时应根据报告修复，再运行相同入口，不能沿用旧 dist 作为本轮成功证据。
...
[实现入口](../../src/integrations/npm/build.js) · [对应测试](../../test/build.test.js)
