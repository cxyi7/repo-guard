...
- 手动命令始终审计当前工作区全量。`changedFiles` 在 pre-push、CI 与交付复核 中比较基线与当前 revision 的“未使用集合”，只阻止新增未引用图片或删除最后一处引用造成的新债务；`allFiles` 阻止全部存量。
...
执行入口：手动、pre-push、CI 与交付复核。功能开关、CI 模式与具体文件范围仍按上文配置生效。
