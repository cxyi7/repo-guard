...
| `checks.imageAssets.naming.enabled` | 是否启用图片名称检查 | `true` / `false`<br>默认：`true` | 使用 JSON 布尔值，不能写成字符串 "true" / "false" |
| `checks.imageAssets.naming.convention` | 图片文件名采用的统一命名风格 | `"camelCase"` / `"kebab-case"`<br>默认：`"camelCase"` | 只接受列出的值 |
| `checks.imageAssets.naming.lowercaseExtension` | 是否要求扩展名使用小写 | `true` / `false`<br>默认：`true` | 使用 JSON 布尔值，不能写成字符串 "true" / "false" |
...
| `checks.imageAssets.compression.minInputBytes` | 开始评估优化的最小输入字节数 | 整数<br>默认：`8192` | ≥ 0；≤ 200000000 |
| `checks.imageAssets.compression.minSavingsBytes` | 优化结果至少需要节省的字节数 | 整数<br>默认：`2048` | ≥ 1；≤ 200000000 |
...
| `checks.imageAssets.compression.conversion.target` | 转换目标格式；当前仅支持 WebP | 只能为 `"webp"`<br>默认：`"webp"` | 只接受列出的值 |
| `checks.imageAssets.compression.conversion.sourceFormats` | 可以转换为 WebP 的原图格式 | 数组；每项可选 `"png"`、`"jpg"`、`"jpeg"`<br>默认：`["png","jpg","jpeg"]` | 至少 1 项；元素不可重复 |
| `checks.imageAssets.compression.conversion.action` | report 报告转换收益；error 将需转换项作为违规 | `"report"` / `"error"`<br>默认：`"report"` | 只接受列出的值 |
| `checks.imageAssets.compression.conversion.minInputBytes` | 开始评估优化的最小输入字节数 | 整数<br>默认：`8192` | ≥ 0；≤ 200000000 |
| `checks.imageAssets.compression.conversion.minSavingsBytes` | 优化结果至少需要节省的字节数 | 整数<br>默认：`4096` | ≥ 1；≤ 200000000 |
| `checks.imageAssets.compression.conversion.minSavingsPercent` | 优化结果至少需要节省的百分比，与字节阈值同时满足 | 整数<br>默认：`20` | ≥ 1；≤ 99 |
| `checks.imageAssets.compression.conversion.pngMode` | lossless 为 PNG 无损转换；lossy 为有损转换 | `"lossless"` / `"lossy"`<br>默认：`"lossless"` | lossy 写入需同时允许有损并传 --allow-lossy。 |
| `checks.imageAssets.compression.conversion.jpegQuality` | JPEG 转 WebP 的编码质量参数；并非压缩率 | 整数<br>默认：`82` | ≥ 1；≤ 100 |
| `checks.imageAssets.compression.conversion.effort` | WebP 编码投入级别，较高值通常需要更多计算时间 | 整数<br>默认：`6` | ≥ 0；≤ 6 |
| `checks.imageAssets.compression.conversion.exactAlpha` | 是否保留透明像素中的 RGB 信息 | `true` / `false`<br>默认：`true` | 使用 JSON 布尔值，不能写成字符串 "true" / "false" |
| `checks.imageAssets.compression.conversion.allowFallbackOriginal` | 转换收益不足时是否允许保留原格式作为退路 | `true` / `false`<br>默认：`false` | 使用 JSON 布尔值，不能写成字符串 "true" / "false" |
| `checks.imageAssets.limits.maxInputBytes` | 单张图片读取输入字节上限 | 整数<br>默认：`26214400` | ≥ 1024；≤ 200000000 |
| `checks.imageAssets.limits.maxPixels` | 单张图片允许解码的像素数量上限 | 整数<br>默认：`40000000` | ≥ 1；≤ 100000000 |
| `checks.imageAssets.limits.maxFrames` | 允许的最大帧数，默认仅单帧图片 | 整数<br>默认：`1` | ≥ 1；≤ 1000 |
...
- 图片命名与 `checks.pathNaming` 同时启用时必须使用同一种 `convention`；图片由 `checks.imageAssets.naming` 检查，避免同一路径被两套规则重复报告。扩展名必须小写，倍率后缀只能使用配置白名单。
