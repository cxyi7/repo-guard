...
多应用仓库的例外统一放在根配置中，`repository.exceptions.entries[].path` 必须是相对 Git 根的完整文件路径，例如 `apps/api/src/runtime.js`。执行应用检查时只使用该应用的批准记录，不会放行其他应用的同名文件；例外标识、批准人和期限保持不变。根目录单应用继续使用 `src/runtime.js` 这样的路径。
