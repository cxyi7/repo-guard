...
Java 工程检查通过独立的工具采集、原生报告解析、策略判断和门禁入口实现。当前支持显式声明的 Maven 项目；Gradle 不在本轮支持范围。已有配置省略开关时保持关闭；新建模板默认开启，不自动安装 Maven、插件或规则，不修改消费项目配置，也不检查业务逻辑。
...
示例字段及未展开的公共选项说明如下。表中“必填”表示对应检查启用后必须显式提供；已有显式关闭配置可以保留空模块清单；新建全开模板的空模块必须补齐。
