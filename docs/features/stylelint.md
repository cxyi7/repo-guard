...
复用项目自己的 Stylelint 和配置，检查 CSS、预处理样式和 Vue 样式。初始化仅在探测到可用设置时启用 Stylelint，并同时启用样式复杂度与样式治理。
...
| `pattern` | 选择暂存文件的 glob，使用项目相对路径匹配 | 字符串<br>默认：`"**/*.{css,scss,sass,less,vue}"` | 至少 1 个字符 |
...
| `governance.enabled` | 是否启用样式优先级与全局位置治理 | `true` / `false`<br>默认：`false` | 使用 JSON 布尔值，不能写成字符串 "true" / "false"； 首次 init 会按项目就绪探测启用；表中是补缺默认值。 启用命令会打开 Stylelint，关闭 Stylelint 会同时关闭本项。 |
| `governance.maxSpecificity` | 优先级上限，顺序为 ID、类/属性/伪类、元素/伪元素 | 字符串<br>默认：`"0,3,0"` | 三个非负整数字段，以逗号分隔且不插入空格，例如 0,3,0 |
| `governance.maxIdSelectors` | 单个选择器允许的 ID 选择器数量；0 表示禁止 | 整数<br>默认：`0` | ≥ 0 |
| `governance.disallowImportant` | 是否禁止 !important 声明 | `true` / `false`<br>默认：`true` | 使用 JSON 布尔值，不能写成字符串 "true" / "false" |
| `governance.allowedGlobalStylePatterns` | 允许全局样式的仓库相对路径；CSS Module 自动按隔离样式处理 | 字符串数组<br>默认：内置 8 项，见[默认配置](../../src/config/defaults.js) | 至少 1 项；元素不可重复；每项为非空字符串 |
...
提交阶段先修复样式，再在格式化完成后复核；CI `full` 只读检查。样式复杂度和样式治理也有手动入口：
