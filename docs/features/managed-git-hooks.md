...
启用的仓库级文件归位读取可信 `revision.head` 的完整提交树，CI 与交付复核 使用相同范围；它不只检查本轮变更，也不按受影响应用裁剪。缺少可信提交、索引冲突或读取错误都会保留相应的范围/执行错误，按公共退出码返回，不能用空清单当作通过。Git 子模块（gitlink）入口及内部文件不参与本仓归位。
