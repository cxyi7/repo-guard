...
[接入](#仓库内合同包接入与配置) · [角色分工](#角色分工与交付时序) · [功能登记](#功能登记与合同规划) · [证据复核](#交付证据与两轮复核) · [反馈升级](#真实测试反馈与反向升级) · [字段格式](#字段与文件参考)
...
接口验收后续归入交付合同：合同开启时默认启用，在候选版本部署到测试或预发布环境后、生产放行前执行。合同明确接口清单、参数来源、认证、测试数据、成功断言、清理及必需场景；Skill 补齐项目场景，程序核对执行证据与候选版本。未执行或跳过不能作为通过。

受控挑拣后续由交付合同约束目标版本、源提交清单、来源映射、冲突处理、依赖复核和挑拣后重新验证。当前只实现提交规范预设禁止 merge commit，不实施受控挑拣，也不声称能证明普通提交使用了 cherry-pick。
