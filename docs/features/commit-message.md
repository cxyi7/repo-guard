...
新建配置默认开启提交信息门禁，禁止 merge commit，默认不强制提升 major。既有配置读取时不改写；省略字段的通用回退值仍保持原行为。显式启用会补齐新预设，用户已填写的值优先。启用后，本地 `commit-msg` 会在自动变更文件摘要定稿前校验人工提交内容；pre-push、CI 与交付复核 会重新读取实际提交对象，校验本次 Git revision 范围，不能只靠跳过本地 Hook 绕过。
...
执行入口：commit-msg、pre-push、CI 与交付复核。功能开关、CI 模式与具体文件范围仍按上文配置生效。
