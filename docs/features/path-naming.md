...
本项文件与目录共用一种风格。Java 类文件的大驼峰、包目录的小写及按目录指定文件后缀，请使用独立的 [Java 文件与目录命名](java-path-naming.md)，避免将 `OrderService.java` 错判为小驼峰违规。
...
- pre-commit、CI 与交付复核 每次检查 Git 索引中的全部已跟踪路径，不只检查本次变更，因此启用前应先完成存量路径治理；新暂存路径也会立即进入检查，已删除路径不再阻断。
