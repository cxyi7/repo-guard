...
test('requires a Stylelint object with valid execution settings', () => {
  assert.throws(
    ...
    /checks\.stylelint 必须是对象/,
  );
  assert.throws(
    () =>
      validateStylelintConfiguration(
        { stylelint: { command: 'lint' } },
        CONFIG_PATH,
      ),
    /包含不支持的属性： command/,
  );
...
test('requires valid Stylelint complexity settings and their parent gate', () => {
  ...
  assert.throws(
    ...
    /styleComplexity\.maxNestingDepth 必须是非负整数/,
  );
  assert.throws(
    () =>
      validateStylelintConfiguration(
...
test('requires valid Stylelint governance settings', () => {
  assert.throws(
    () =>
      validateStylelintConfiguration(
        { stylelint: { governance: [] } },
        CONFIG_PATH,
      ),
    /checks\.styleGovernance 必须是对象/,
  );
  assert.throws(
    () =>
      validateStylelintConfiguration(
