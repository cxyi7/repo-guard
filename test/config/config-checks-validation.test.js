...
import { DEFAULT_ESLINT_CONFIG, DEFAULT_PRETTIER_CONFIG, DEFAULT_MAX_FILE_LINES_CONFIG } from '../../src/config/defaults.js';
...
test('applies all engineering defaults when checks are omitted', () => {
  ...
  assert.deepEqual(Object.keys(checks).sort(), Object.keys(PROJECT_CHECK_PATHS).sort());
...
test('delegates checks to their domain validators without reintroducing preCommit', () => {
  ...
  for (const name of ['asyncResourceCleanup', 'fileHeader', 'functionDocs', 'stylelint', 'pathNaming']) {
    assert.equal(checks[name].enabled, true);
  }
  for (const name of ['eslint', 'prettier', 'maxFileLines', 'filePlacement']) assert.equal(checks[name].enabled, false);
...
test('checks reject unknown fields, invalid sections and duplicate nested features', () => {
  assert.throws(() => validateChecksConfiguration([], PROJECT, CONFIG_PATH), /checks 必须是对象/);
  assert.throws(() => validateChecksConfiguration({
    invented: {}
  }, PROJECT, CONFIG_PATH), /包含不支持的属性： invented/);
  assert.throws(() => validateChecksConfiguration({
    unitTest: {
      coverage: {}
    }
  }, PROJECT, CONFIG_PATH), /已移动为 checks.coverage/);
  assert.throws(() => validateChecksConfiguration({
    stylelint: {
      complexity: {}
    }
  }, PROJECT, CONFIG_PATH), /不支持的属性： complexity/);
});
