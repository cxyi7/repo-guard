...
import { configureCi, ensureProjectConfig, setFeaturesEnabled } from '../../src/orchestration/setup/config-management.js';
...
function read(root) {
  return JSON.parse(readFileSync(path.join(root, 'repo-guard.config.json'), 'utf8'));
}
...
test('多应用只改所选应用，公共 CI 独立写根', t => {
  ...
  setFeaturesEnabled(root, ['unitTest'], true, {
    projectId: 'api'
  });
...
test('多应用 UI 契约与构建基线保护写入应用策略，保留既有仓库规则', t => {
  ...
  setFeaturesEnabled(root, ['stylelint'], true, {
    projectId: 'web'
  });
