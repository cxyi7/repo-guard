import { projectFixtureDocument, stringifyProjectFixture } from '../../helpers/project-config.js';
...
test('Git 增量门禁分别使用基线与当前配置，阻断移除别名造成的新债务', (context) => {
  ...
  writeFileSync(
    ...
    `${stringifyProjectFixture(baselineConfig, null, 2)}\n`,
  );
  runGit(['add', '.'], { cwd: root });
...
test('多应用图片增量检查从基线清单读取自定义子配置，忽略工作区的别名修改', (context) => {
  const root = gitFixture(context);
  ...
  const { rules, exclusions, commitMessage, deliveryContract, filePlacement, ...applicationRepository } = repository;
  ...
  writeFileSync(path.join(root, 'repo-guard.config.json'), JSON.stringify({
    ...
    repository: { commitMessage, deliveryContract, filePlacement }, reporting, ci: sharedCi,
  }));
  writeFileSync(appConfigFile, JSON.stringify(appDocument));
  writeFileSync(path.join(appRoot, 'src', 'assets', 'logo.png'), 'logo');
  writeFileSync(path.join(appRoot, 'src', 'pages', 'home.ts'), "import logo from '@/assets/logo.png';");
  runGit(['add', '.'], { cwd: root });
  runGit(['commit', '-m', 'test: 建立多应用图片基线'], { cwd: root });
  const base = runGit(['rev-parse', 'HEAD'], { cwd: root }).stdout.trim();
  appDocument.checks.unusedImageAssets.aliases = [];
  writeFileSync(appConfigFile, JSON.stringify(appDocument));
  runGit(['add', '.'], { cwd: root });
  runGit(['commit', '-m', 'test: 删除应用别名'], { cwd: root });
  const head = runGit(['rev-parse', 'HEAD'], { cwd: root }).stdout.trim();
  const { config } = loadWorkspace(root).projects[0];
  const plan = unusedImageAssetsGate.plan({ config, environment: 'ci-full', files: [], revision: { base, head } });
  const result = unusedImageAssetsGate.run({ root: appRoot, repositoryRoot: root, config, plan });
  assert.equal(result.status, 'violation', result.summary);
  assert.deepEqual(result.findings.map(({ location }) => location.path), ['src/assets/logo.png']);
});
