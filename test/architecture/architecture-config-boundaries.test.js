...
test('keeps shared configuration validation primitives in the config module', () => {
  ...
  const configurationLoaderSource = readFileSync(path.join(SOURCE_ROOT, 'config', 'workspace-configuration.js'), 'utf8');
...
test('keeps configuration validation and loading in config modules without a root facade', () => {
  ...
  const workspaceSource = readFileSync(path.join(SOURCE_ROOT, 'config', 'workspace-configuration.js'), 'utf8');
  const projectSource = readFileSync(path.join(SOURCE_ROOT, 'config', 'project-configuration.js'), 'utf8');
  assert.match(projectSource, /from ['"]\.\/configuration-validation\.js['"]/);
  assert.match(workspaceSource, /JSON\.parse\(readFileSync/);
  assert.match(workspaceSource, /assertExceptionLifecycleCurrent/);
  assert.match(
    workspaceSource,
