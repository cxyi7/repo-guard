...
test('enforces every declared platform dependency direction as an error', async () => {
  ...
  try {
    ...
    writeFixture(fixtureRoot, 'src/operations/invalid.js', "import '../orchestration/runner.js';\n");
    writeFixture(fixtureRoot, 'src/profiles/invalid.js', "import '../gates/testing/gate.js';\n");
...
test('keeps managed GitLab CI files in operations with a configuration-only setup bridge', () => {
  ...
  assert.doesNotMatch(gitLabCiSource, /from ['"][^'"]*orchestration\//);
});
...
test('separates GitLab notification policy, release delivery, package facts, and CLI wiring', () => {
  ...
  const gatePath = path.join(SOURCE_ROOT, 'operations', 'notifications', 'gitlab-ci-notification.js');
...
test('keeps package exports on reviewed contracts and schemas', () => {
  ...
  assert.deepEqual(packageJson.exports, {
    ...
    './operations.schema.json': './operations.schema.json',
...
test('keeps doctor repository mutation in setup orchestration separate from diagnosis', () => {
  ...
  assert.match(repairSource, /from ['"]\.\.\/\.\.\/config\/configuration-loader\.js['"]/);
  assert.doesNotMatch(repairSource, /ensureProjectConfig|migrateProjectConfig/);
