...
import { agentPolicyCatalog, agentPolicyGroups, managedAgentPolicyCapabilities, managedAgentPolicyFeatures, managedAgentPolicyGateIds, renderAgentPolicyGroups } from '../../src/policies/agent-policy-catalog.js';
test('根规范说明应用隔离与独立合同，不要求在公共入口登记应用例外', () => {
  const config = createStarterConfig();
  delete config.project;
  const document = renderAgentPolicyGroups({
    config,
    packageJson: {}
  }).flatMap(({
    lines
...
test('仓库归位生成根 AI 规范并明确范围，应用默认不复制根规则', () => {
  ...
  const applicationDocument = renderAgentPolicyGroups({
...
import { agentPolicies, inspectAgentPolicies, syncAgentPolicies } from '../../src/policies/agent-policies.js';
const TEST_ROOT = path.join(process.cwd(), 'test', '.tmp');
mkdirSync(TEST_ROOT, {
  recursive: true
});
test('UI Token 托管规范随语言配置同步并说明变量来源限制', () => {
  ...
  const config = {
    ...
    checks: {
      ...
      "stylelint": {
        "uiTokens": {
          ...starter.checks.stylelint.uiTokens,
          enabled: true,
  ...
  assert.doesNotMatch(document, /图标尺寸|iconSelectors/);
...
test('每个可配置功能和既有官方门禁都归入托管规范目录', () => {
  const groupIds = agentPolicyGroups.map(({
    id
  }) => id);
  const entryIds = agentPolicyCatalog.map(({
    id
  }) => id);
  assert.equal(new Set(groupIds).size, groupIds.length);
  assert.equal(new Set(entryIds).size, entryIds.length);
  assert.ok(agentPolicyCatalog.every(({
    groupId
  ...
  assert.deepEqual([...managedAgentPolicyFeatures].sort(), [...CONFIGURABLE_FEATURES].sort());
  assert.deepEqual([...managedAgentPolicyGateIds].sort(), officialGates.map(({
    id
  }) => id).filter(id => id !== 'repository.agent-policy').sort());
  assert.deepEqual(agentPolicies.map(({
    id
  }) => id), agentPolicyGroups.map(({
    id
  }) => id));
  assert.deepEqual(managedAgentPolicyCapabilities, ['dead-code-baseline', 'guarded-build', 'build-artifact-baseline', 'external-gates', 'api-performance', 'k6']);
});
test('保留人工内容并按当前配置增删托管规则，重复同步保持幂等', context => {
  ...
  writeFileSync(path.join(root, 'AGENTS.md'), ['# 人工规范', '', '这里的内容由项目维护。', '', '另一段人工内容也必须保留。', ''].join('\n'));
  const config = createStarterConfig();
  ...
  assert.ok(enabledContent.indexOf('# 人工规范') < enabledContent.indexOf('另一段人工内容也必须保留。'));
  assert.ok(enabledContent.indexOf('另一段人工内容也必须保留。') < enabledContent.indexOf('<!-- repo-guard:repository-governance-policy:start -->'));
...
test('旧版、未知和新旧混合规范 marker 均拒绝，不改原文件或追加当前区块', context => {
  ...
  for (const id of ['architecture-policy', 'exception-policy', 'unit-test-policy', 'accessibility-test-policy', 'unknown-policy']) {
    ...
    for (const content of [unsupported, `${current}\n${unsupported}`, unsupported.replaceAll('\n', '\r\n')]) {
      ...
      assert.throws(() => syncAgentPolicies(root, config), {
        code: 'managed-text/unsupported-markers'
      });
      assert.throws(() => inspectAgentPolicies(root, config), {
        code: 'managed-text/unsupported-markers'
      });
...
test('marker 异常时拒绝任何写入', context => {
  ...
  const malformed = ['<!-- repo-guard:testing-policy:start -->', '缺少结束标记', ''].join('\n');
  writeFileSync(path.join(root, 'AGENTS.md'), malformed);
  assert.throws(() => syncAgentPolicies(root, createStarterConfig()), error => error.code === 'managed-text/malformed-markers');
...
test('启用和禁用图片治理会同步 AGENTS.md 托管规则', context => {
  ...
  writeFileSync(path.join(root, 'repo-guard.config.json'), `${stringifyProjectFixture(createStarterConfig(), null, 2)}\n`, 'utf8');
...
test('启用和禁用交付合同会同步项目级流程 Skills', context => {
  ...
  assert.equal(existsSync(path.join(root, '.agents', 'skills', 'repo-guard-delivery-contract', 'SKILL.md')), true);
  ...
  assert.equal(existsSync(path.join(root, '.agents', 'skills', 'repo-guard-delivery-contract', 'SKILL.md')), false);
});
test('启用无效图片资源会把静态引用和动态声明边界写入 AGENTS.md', context => {
  ...
  const enabledContent = readFileSync(path.join(root, 'AGENTS.md'), 'utf8');
  assert.match(enabledContent, /无效图片资源按 `allFiles` 模式治理/);
...
test('托管 AI 规范不再要求表单标签或图片替代文本', () => {
  const document = renderAgentPolicyGroups({ config: createStarterConfig(), packageJson: {} })
    .flatMap(({ lines }) => lines).join('\n');
  assert.doesNotMatch(document, /Vue 原生表单控件|Vue 原生图片|装饰图片必须/);
  assert.equal(agentPolicyCatalog.some(({ id }) => id === 'vue-accessibility'), false);
});
