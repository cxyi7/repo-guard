...
import { renderExceptionRegistrySummary } from '../../src/core/report/exception-registry-renderer.js';
...
test('classifies active, expiring, expired, and future-dated exceptions', () => {
  const result = inspectExceptionLifecycle(
    registry([
  ...
  assert.deepEqual(
    result.entries.map(({ status }) => status),
    ['active', 'expiring', 'expired', 'future'],
  );
  assert.match(
    renderExceptionRegistrySummary(result),
