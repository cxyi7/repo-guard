...
test('starter configuration enables standard gates and leaves Stylelint opt-in', () => {
  ...
  assert.deepEqual(config.checks.stylelint.uiTokens.languages, ['css']);
  assert.equal(
    Object.hasOwn(config.checks.stylelint.uiTokens, 'adapters'),
    false,
  );
...
test('统一主开关保留 Token 配置并维护清单保护', (context) => {
  const root = createFixture(
    sparseConfig({
      checks: {
        stylelint: {
          enabled: false,
          uiTokens: { enabled: true, languages: ['css', 'sass', 'less'] },
        },
      },
    }),
  );
  context.after(() => rmSync(root, { recursive: true, force: true }));
  assert.deepEqual(setFeaturesEnabled(root, ['stylelint'], true).changed, [
    'stylelint',
  ]);
  const config = readConfig(root);
  assert.equal(config.checks.stylelint.uiTokens.enabled, true);
  assert.deepEqual(config.checks.stylelint.uiTokens.languages, [
    'css',
    'sass',
    'less',
  ]);
  assert.ok(
    config.repository.rules.some(
      (r) => r.pattern === 'ui-tokens.manifest.json',
    ),
  );
  setFeaturesEnabled(root, ['stylelint'], false);
  assert.equal(readConfig(root).checks.stylelint.uiTokens.enabled, true);
  assert.equal(readConfig(root).checks.stylelint.enabled, false);
});
...
test('旧独立功能 styleComplexity 被明确拒绝', (context) => {
  const root = createFixture(sparseConfig());
  context.after(() => rmSync(root, { recursive: true, force: true }));
  assert.throws(
    () => setFeaturesEnabled(root, ['styleComplexity'], true),
    /不支持的功能/,
  );
});
test('旧独立功能 styleGovernance 被明确拒绝', (context) => {
  const root = createFixture(sparseConfig());
  context.after(() => rmSync(root, { recursive: true, force: true }));
  assert.throws(
    () => setFeaturesEnabled(root, ['styleGovernance'], true),
    /不支持的功能/,
  );
});
test('关闭 Stylelint 保留子配置', (context) => {
  const root = createFixture(
    sparseConfig({
      checks: {
        stylelint: {
  ...
  assert.deepEqual(disabled.changed, ['stylelint']);
  const config = readConfig(root);
  assert.equal(config.checks.stylelint.enabled, false);
  assert.equal(config.checks.stylelint.governance.enabled, true);
});
...
test('重复开启无效代码检查保留用户范围和覆盖值，关闭不删除配置', (t) => {
  const root = createFixture(sparseConfig({ checks: { deadCode: { enabled: false, options: { entry: ['client/launch.js'], project: ['client/**/*.js'], ignoreDependencies: ['external-runtime'], includeEntryExports: true } } } }));
  t.after(() => rmSync(root, { recursive: true, force: true }));
  setFeaturesEnabled(root, ['deadCode'], true);
  const first = readConfig(root).checks.deadCode;
  assert.equal(first.enabled, true);
  assert.deepEqual(first.options.entry, ['client/launch.js']);
  assert.deepEqual(first.options.ignoreDependencies, ['external-runtime']);
  assert.equal(first.options.includeEntryExports, true);
  assert.equal(first.options.ignoreExportsUsedInFile, false);
  setFeaturesEnabled(root, ['deadCode'], true);
  assert.deepEqual(readConfig(root).checks.deadCode, first);
  setFeaturesEnabled(root, ['deadCode'], false);
  const disabled = readConfig(root).checks.deadCode;
  assert.equal(disabled.enabled, false);
  assert.deepEqual(disabled.options, first.options);
});

test('提交规范启用补齐预设且保留用户覆盖，重复执行与关闭不丢配置', (t) => {
  const root = createFixture(sparseConfig());
  t.after(() => rmSync(root, { recursive: true, force: true }));
  writeFileSync(path.join(root, CONFIG_FILE), JSON.stringify(sparseConfig()));
  setFeaturesEnabled(root, ['commitMessage'], true);
  let config = readConfig(root);
  assert.equal(config.repository.commitMessage.enabled, true);
  assert.equal(config.repository.commitMessage.merge.allowed, false);
  assert.equal(config.repository.commitMessage.breakingChange.requireMajorVersionOnRelease, false);
  config.repository.commitMessage.merge.allowed = true;
  config.repository.commitMessage.headerMaxLength = 72;
  writeFileSync(path.join(root, CONFIG_FILE), stringifyProjectFixture(config));
  setFeaturesEnabled(root, ['commitMessage'], true);
  const before = readConfig(root).repository.commitMessage;
  assert.equal(before.merge.allowed, true);
  assert.equal(before.headerMaxLength, 72);
  assert.deepEqual(setFeaturesEnabled(root, ['commitMessage'], true).unchanged, ['commitMessage']);
  setFeaturesEnabled(root, ['commitMessage'], false);
  assert.deepEqual(readConfig(root).repository.commitMessage, { ...before, enabled: false });
});
