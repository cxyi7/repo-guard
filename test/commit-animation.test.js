...
test('动画配置默认关闭，旧配置迁移和功能开关保留主题', (t) => {
  ...
  assert.equal(migrateProjectConfig(root).config.commitAnimation.theme, 'dog');
  setFeaturesEnabled(root, ['commitAnimation'], true);
  assert.equal(JSON.parse(readFileSync(file)).commitAnimation.enabled, true);
  setFeaturesEnabled(root, ['commitAnimation'], false);
  assert.equal(JSON.parse(readFileSync(file)).commitAnimation.theme, 'dog');
});
...
test('拒绝信号主题、未知字段、非法概率及非布尔开关', () => {
  ...
  assert.deepEqual(schema.properties.commitAnimation.properties.theme.enum, [
    'cat',
    'dog',
  ]);
  assert.deepEqual(
    Object.keys(schema.properties.commitAnimation.properties).sort(),
