...
function fixture(context, changedFile) {
  ...
  for (const id of ['web', 'api']) {
    files[`apps/${id}/package.json`] = json({
      name: id,
      type: 'module'
    });
...
for (const changedFile of [WEB_CONFIG, ROOT_CONFIG]) {
  test(`CI 增量范围只含 ${changedFile} 时仍报告未改动样式的 Token 违规`, async context => {
    ...
    assert.equal(workspace.projects.find(({
      id
    }) => id === 'web').config.ci.gatePolicy.gates['quality.ui-tokens'].scope, 'changed-files');
    const exitCode = await runWorkspaceCi({
      workspace,
      options: {
        base,
        head,
    ...
    const web = report.targets.find(({
      projectId
    }) => projectId === 'web');
    const step = web.report.steps.find(({
      gateResult
    }) => gateResult?.gateId === 'quality.ui-tokens');
    ...
    assert.deepEqual(step.gateResult.findings.map(({
      ruleId,
      location
    ...
    if (changedFile === WEB_CONFIG) {
      ...
      const apiTokenStep = api.report.steps.find(({
        gateResult
      }) => gateResult?.gateId === 'quality.ui-tokens');
