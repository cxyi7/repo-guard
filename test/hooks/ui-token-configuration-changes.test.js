...
function stagedTargets(workspace, environment = 'pre-commit') {
  return createWorkspaceTargets({
    ...
    changes: createChangeSet({
      source: 'staged',
      changes: collectStagedChanges(workspace.root)
    }),
    files: []
  });
}
for (const configFile of [WEB_CONFIG, ROOT_CONFIG]) {
  test(`只暂存 ${configFile} 仍复查未更改样式并阻断原始值`, async context => {
    ...
    const web = stagedTargets(workspace).projects.find(({
      project
    }) => project.id === 'web');
    assert.equal(web.configurationChanged, true);
    assert.deepEqual(web.files, []);
    assert.deepEqual(uiTokenGate.plan(web).files.map(({
      relative
    }) => relative), ['src/card.css']);
    assert.deepEqual(collectStagedChanges(root).map(({
      path: file
    }) => file), [configFile]);
    ...
    assert.deepEqual(execution.decisiveResult.findings.map(({
      ruleId,
      location
    }) => [ruleId, location.path]), [['ui-token/raw-value', 'src/card.css']]);
...
test('只修改后端配置不会复查未受影响前端的样式', async context => {
  ...
  const workspace = loadWorkspace(root);
  const web = stagedTargets(workspace, 'ci-policy').projects.find(({
    project
  }) => project.id === 'web');
  assert.equal(web.configurationChanged, false);
  assert.deepEqual(uiTokenGate.plan(web).files, []);
  assert.equal(stagedTargets(workspace).projects.some(({
    project
  }) => project.id === 'web'), false);
  const execution = await runWorkspaceQualityExecution(workspace, []);
  assert.equal(execution.exitCode, 0);
  assert.equal(execution.results.some(({
    gateId
  }) => gateId === 'quality.ui-tokens'), false);
});
  ...
  test(`配置重命名来源 ${previousConfig} 仍触发前端样式复查`, context => {
    ...
    const targets = createWorkspaceTargets({
      ...
      changes: createChangeSet({
        ...
        changes: [{
          status: 'R100',
          oldPath: previousConfig,
          path: `${previousConfig}.saved`
        }]
      }),
      files: []
    });
    const web = targets.projects.find(({
      project
    }) => project.id === 'web');
    assert.equal(web.configurationChanged, true);
    assert.deepEqual(uiTokenGate.plan(web).files.map(({
...
test('同目录未声明的默认配置文件名不触发自定义配置应用的全量复查', context => {
  ...
  const targets = createWorkspaceTargets({
    ...
    changes: createChangeSet({
      ...
      changes: [{
        status: 'M',
        path: 'apps/web/repo-guard.config.json'
      }]
    }),
    files: []
  });
