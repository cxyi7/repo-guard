...
function writeManifest(root, source) {
  writeFileSync(path.join(root, 'ui-tokens.manifest.json'), `${JSON.stringify(tokenManifest(source), null, 2)}\n`);
}
...
test('pre-commit 在只有删除项时仍按暂存快照阻断移除 Manifest 来源', async context => {
  ...
  writeFileSync(path.join(root, 'repo-guard.config.json'), `${stringifyProjectFixture(projectConfiguration(), null, 2)}\n`);
  git(root, ['add', '.']);
  ...
  const execution = await runQualityExecution({
    root,
    files: [],
    config: validateConfig(projectConfiguration())
  });
  assert.equal(execution.decisiveResult.gateId, 'quality.ui-tokens');
  assert.equal(execution.decisiveResult.findings[0].ruleId, 'ui-token/stale-manifest');
