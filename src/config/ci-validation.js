...
export function validateCiConfiguration(value, configPath) {
  const ciValue = value === undefined ? {} : value;
  ...
  const ciProtectedFilesAction =
    ciProtectedFilesValue.action ?? DEFAULT_CI_CONFIG.protectedFiles.action;
  if (!['report', 'fail'].includes(ciProtectedFilesAction)) {
    throw configValidationError(
      `${configPath} ci.protectedFiles.action 必须为 report 或 fail`,
    );
  }
  const gatePolicyValue = ciValue.gatePolicy ?? {};
  if (
    !gatePolicyValue ||
    typeof gatePolicyValue !== 'object' ||
  ...
  if (
    !gatePoliciesValue ||
    typeof gatePoliciesValue !== 'object' ||
  ...
  for (const [gateId, policy] of Object.entries(gatePoliciesValue)) {
    ...
    if (!/^[a-z][a-z0-9-]*(?:\.[a-z][a-z0-9-]*)+$/.test(gateId)) {
      throw configValidationError(
        `${label} 的门禁 id 必须使用点分隔的 kebab-case 命名`,
      );
    }
    ...
    if (!CI_GATE_SCOPES.includes(scope)) {
      throw configValidationError(
        `${label}.scope 必须为 all-files 或 changed-files`,
      );
    }
    gatePolicies[gateId] = { mode: policy.mode, scope };
  }

  const externalGatesValue =
    ciValue.externalGates === undefined ? [] : ciValue.externalGates;
  if (!Array.isArray(externalGatesValue)) {
    throw configValidationError(`${configPath} ci.externalGates 必须是数组`);
  }
  const externalGateIds = new Set();
  const externalReportPaths = new Set();
  const externalGates = externalGatesValue.map((entry, index) => {
    const label = `${configPath} ci.externalGates 第 ${index + 1}`;
    ...
    for (const field of [
      ...
      if (!Object.hasOwn(entry, field))
        throw configValidationError(`${label}.${field} 为必填项`);
    }
    if (
      typeof entry.id !== 'string' ||
      !/^project\.[a-z][a-z0-9-]*$/.test(entry.id)
    ) {
      throw configValidationError(
        `${label}.id 必须使用 project.<kebab-case> 命名空间`,
      );
    }
    if (externalGateIds.has(entry.id)) {
      throw configValidationError(
        `${configPath} 外部门禁 id 重复： ${entry.id}`,
      );
    }
    ...
    if (
      ...
      entry.environments.some(
        (environment) =>
          !['manual', 'ci-full', 'release-ready'].includes(environment),
      ) ||
      new Set(entry.environments).size !== entry.environments.length
    ) {
      throw configValidationError(
        `${label}.environments 只能包含不重复的 manual、ci-full 或 release-ready 值`,
      );
    }
    if (
      typeof entry.script !== 'string' ||
      !/^[A-Za-z0-9:_-]+$/.test(entry.script)
    ) {
    ...
    if (
      ...
      entry.timeoutMs > 1800000
    ) {
      throw configValidationError(
        `${label}.timeoutMs 必须介于 1000 到 1800000 之间`,
      );
    }
    if (
      !entry.report ||
    ...
    if (entry.report.format !== 'repo-guard-json-v2') {
      throw configValidationError(
        `${label}.report.format 必须为 repo-guard-json-v2`,
      );
    }
    const reportSegments =
      typeof entry.report.path === 'string' ? entry.report.path.split('/') : [];
    if (
      reportSegments[0] !== 'reports' ||
      ...
      throw configValidationError(
        `${label}.report.path 必须使用 reports/ 内的规范化路径`,
      );
    }
    const reportPath = validateCiReportPath(
      entry.report.path,
      `${label}.report.path`,
    );
  ...
  if (externalReportPaths.has(ciReportPath.toLowerCase())) {
    throw configValidationError(
      `${configPath} 外部门禁报告路径不能与 ci.reportPath 相同`,
    );
  }
