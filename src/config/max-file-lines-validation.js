...
function normalizeMaxFileLineRule(rule, index, configPath) {
  const label = `${configPath} checks.maxFileLines 规则 ${index + 1}`;
...
function normalizeMaxFileLineExclusions(exclusionsValue, configPath) {
  if (!Array.isArray(exclusionsValue)) {
    throw configValidationError(`${configPath} checks.maxFileLines.exclusions 必须是数组`);
  }
  return exclusionsValue.map((pattern, index) => {
    if (typeof pattern !== 'string' || !pattern.trim()) {
      throw configValidationError(
        `${configPath} checks.maxFileLines 排除项 ${index + 1} 必须是非空字符串`,
      );
    }
...
export function validateMaxFileLinesConfiguration(checksValue, configPath) {
  const maxFileLinesValue = checksValue.maxFileLines ?? {};
  if (
    !maxFileLinesValue
    || typeof maxFileLinesValue !== 'object'
    || Array.isArray(maxFileLinesValue)
  ) {
    throw configValidationError(`${configPath} checks.maxFileLines 必须是对象`);
  }
  assertKnownProperties(
    maxFileLinesValue,
    new Set(['enabled', 'mode', 'warnAt', 'rules', 'exclusions']),
    `${configPath} checks.maxFileLines`,
  );
  if (
    maxFileLinesValue.enabled != null
    && typeof maxFileLinesValue.enabled !== 'boolean'
  ) {
    throw configValidationError(`${configPath} checks.maxFileLines.enabled 必须是布尔值`);
  }
  if (
    ...
    throw configValidationError(
      `${configPath} checks.maxFileLines.mode 必须为 strict 或 noRegression`,
    );
  }
  if (
    ...
    throw configValidationError(`${configPath} checks.maxFileLines.warnAt 必须大于 0 且不超过 1`);
  }
  ...
  if (!Array.isArray(rulesValue) || rulesValue.length === 0) {
    throw configValidationError(`${configPath} checks.maxFileLines.rules 必须是非空数组`);
  }
