...
function normalizeExtensions(value, label) {
  ...
  const supported = new Set(SUPPORTED_FILE_HEADER_EXTENSIONS);
  const normalized = value.map((extension, index) => {
    if (typeof extension !== 'string' || !supported.has(extension)) {
      throw configValidationError(
        `${label} 第 ${index + 1} 项必须是受支持的文件扩展名：${SUPPORTED_FILE_HEADER_EXTENSIONS.join(', ')}`,
      );
    }
    return extension;
  });
...
export function validateFileHeaderConfiguration(preCommitValue, configPath) {
  ...
  return {
    enabled: value.enabled ?? DEFAULT_FILE_HEADER_CONFIG.enabled,
    include: normalizePatternList(
      value.include ?? DEFAULT_FILE_HEADER_CONFIG.include,
      `${label}.include`,
    ),
    exclude: normalizePatternList(
      value.exclude ?? DEFAULT_FILE_HEADER_CONFIG.exclude,
      `${label}.exclude`,
      { allowEmpty: true },
    ),
    extensions: normalizeExtensions(
      value.extensions ?? DEFAULT_FILE_HEADER_CONFIG.extensions,
      `${label}.extensions`,
    ),
  };
}
