...
import { evaluateBuildArtifactBudget } from './build-artifact-budget.js';
...
export async function runBuildGate({
  ...
  let sentinelSetup = null;
  if (config.artifactBudget?.enabled) {
    resolveBuildArtifactOutput(root, config.artifactBudget);
    sentinelSetup = createStaleOutputSentinel(root, config.artifactBudget);
    let cleanExecution;
    try {
  ...
  let execution;
  try {
    ({ execution } = await executeProjectBuild({
      root,
      config,
      signal,
      output,
  ...
  if (config.artifactBudget?.enabled) {
    ...
    return createGateResult({
      ...
      artifacts: [{
        ...
        description: `${config.artifactBudget.platform === 'pc' ? '桌面网页' : '小程序'}构建产物`,
      }],
      metrics: {
        ...evaluation.inspection.metrics,
        baselineDebt: evaluation.baselineDebt,
      },
      durationMs: Date.now() - startedAt,
    });
  }
