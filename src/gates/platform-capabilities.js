...
function placeholderRun() {
  throw new Error('Numeric capability must be invoked through its orchestration adapter');
}

function numericSetupInspection() {
  return null;
}
...
function defineNumericCapability({
  id,
  configKey = null,
  featureName = null,
  featureOrder = null,
  ...
  artifactTypes = [],
  supportsFix = false,
  inspectSetup = numericSetupInspection,
}) {
  return defineGate({
    id,
    configKey,
...
export const platformCapabilities = Object.freeze([
  defineNumericCapability({ id: 'quality.stylelint', configKey: 'preCommit.stylelint', featureName: 'stylelint', featureOrder: 30, doctorOrder: 160, environments: ['pre-commit', 'ci-full'], mutation: 'working-tree-fix', allowedMutations: ['working-tree-fix', 'read-only'], before: ['quality.eslint'], requiredTools: ['stylelint'], supportsFix: true, inspectSetup: setup.stylelint }),
  defineNumericCapability({ id: 'quality.eslint', configKey: 'preCommit.eslint', featureName: 'eslint', featureOrder: 10, doctorOrder: 130, environments: ['pre-commit', 'ci-full'], mutation: 'working-tree-fix', allowedMutations: ['working-tree-fix', 'read-only'], before: ['quality.prettier'], requiredTools: ['eslint'], supportsFix: true, inspectSetup: setup.eslint }),
  defineNumericCapability({ id: 'quality.prettier', configKey: 'preCommit.prettier', featureName: 'prettier', featureOrder: 20, doctorOrder: 170, environments: ['pre-commit', 'ci-full'], mutation: 'working-tree-fix', allowedMutations: ['working-tree-fix', 'read-only'], requiredTools: ['prettier'], supportsFix: true, inspectSetup: setup.prettier }),
  defineNumericCapability({ id: 'security.vue-unsafe-html', doctorOrder: 80, environments: ['manual', 'pre-commit', 'ci-policy', 'ci-full'], manualCommand: 'unsafe-html', manualOrder: 80, packageScript: 'guard:unsafe-html', inspectSetup: () => ready('Vue v-html staged gate (hard requirement, rule=vue/no-v-html)') }),
  defineNumericCapability({ id: 'security.vue-target-blank', doctorOrder: 90, environments: ['manual', 'pre-commit', 'ci-policy', 'ci-full'], manualCommand: 'target-blank', manualOrder: 90, packageScript: 'guard:target-blank', inspectSetup: () => ready('Vue target=_blank staged gate (hard requirement, rel=noopener+noreferrer, rule=vue/target-blank-security)') }),
  defineNumericCapability({ id: 'accessibility.vue-form-label', doctorOrder: 100, environments: ['manual', 'pre-commit', 'ci-policy', 'ci-full'], manualCommand: 'form-labels', manualOrder: 100, packageScript: 'guard:form-labels', inspectSetup: () => ready('Vue form control label staged gate (hard requirement, rule=vue/form-control-label)') }),
  defineNumericCapability({ id: 'accessibility.vue-image-alt', doctorOrder: 110, environments: ['manual', 'pre-commit', 'ci-policy', 'ci-full'], manualCommand: 'image-alt', manualOrder: 110, packageScript: 'guard:image-alt', inspectSetup: () => ready('Vue image alt staged gate (hard requirement, rule=vue/img-alt)') }),
  defineNumericCapability({ id: 'repository.maximum-file-lines', configKey: 'preCommit.maxFileLines', featureName: 'maxFileLines', featureOrder: 50, doctorOrder: 140, environments: ['pre-commit', 'ci-policy', 'ci-full'], inspectSetup: ({ config }) => ready(config.preCommit.maxFileLines.enabled ? `Maximum file lines staged gate (mode=${config.preCommit.maxFileLines.mode}, warnAt=${config.preCommit.maxFileLines.warnAt}, ${config.preCommit.maxFileLines.rules.map(({ pattern, maxLines }) => `${pattern}<=${maxLines}`).join(', ')})` : 'Maximum file lines staged gate is disabled') }),
  defineNumericCapability({ id: 'repository.file-placement', configKey: 'preCommit.filePlacement', featureName: 'filePlacement', featureOrder: 40, doctorOrder: 150, environments: ['manual', 'pre-commit', 'ci-policy', 'ci-full'], manualCommand: 'file-placement', manualOrder: 150, packageScript: 'guard:file-placement', inspectSetup: ({ config }) => ready(config.preCommit.filePlacement.enabled ? `File placement staged gate (mode=${config.preCommit.filePlacement.mode}, ${config.preCommit.filePlacement.rules.length} rules)` : 'File placement staged gate is disabled') }),
  defineNumericCapability({ id: 'dependencies.policy', configKey: 'dependencyPolicy', featureName: 'dependencies', featureOrder: 80, doctorOrder: 120, environments: ['manual', 'pre-commit', 'ci-policy', 'ci-full'], manualCommand: 'dependencies', manualOrder: 20, packageScript: 'guard:dependencies', inspectSetup: ({ config }) => ready(config.dependencyPolicy.enabled ? `Dependency policy (exact=${config.dependencyPolicy.requireExactVersions}, lockfile=${config.dependencyPolicy.requireLockfile}, protocols=${config.dependencyPolicy.allowedProtocols.join(',') || 'none'}, banned=${config.dependencyPolicy.bannedPackages.length})` : 'Dependency policy staged gate is disabled') }),
  defineNumericCapability({ id: 'repository.protected-files', environments: ['pre-commit', 'ci-policy', 'ci-full'], mutation: 'external-write', allowedMutations: ['external-write', 'read-only'] }),
  defineNumericCapability({ id: 'repository.structured-exceptions', configKey: 'exceptions', environments: ['manual', 'ci-policy', 'ci-full'], manualCommand: 'exceptions', manualOrder: 10, packageScript: 'guard:exceptions' }),
  defineNumericCapability({ id: 'quality.typecheck', configKey: 'typeCheck', featureName: 'typeCheck', featureOrder: 130, doctorOrder: 40, environments: ['manual', 'pre-push', 'ci-full'], defaultTimeoutMs: DEFAULT_TYPE_CHECK_CONFIG.timeoutMs, manualCommand: 'typecheck', manualOrder: 50, packageScript: 'guard:typecheck', requiredScripts: ['config:typeCheck.script'], inspectSetup: setup.typecheck }),
  defineNumericCapability({ id: 'quality.unit-test', configKey: 'unitTest', featureName: 'unitTest', featureOrder: 140, doctorOrder: 50, environments: ['manual', 'pre-push', 'ci-policy', 'ci-full'], defaultTimeoutMs: DEFAULT_UNIT_TEST_CONFIG.timeoutMs, manualCommand: 'unit-test', manualOrder: 60, packageScript: 'guard:unit-test', requiredTools: ['vitest'], requiredScripts: ['config:unitTest.script'], artifactTypes: ['coverage-report'], inspectSetup: setup.unitTest }),
  defineNumericCapability({ id: 'quality.accessibility-test', configKey: 'accessibilityTest', featureName: 'accessibilityTest', featureOrder: 100, doctorOrder: 60, environments: ['manual', 'pre-push', 'ci-full'], defaultTimeoutMs: DEFAULT_ACCESSIBILITY_TEST_CONFIG.timeoutMs, manualCommand: 'accessibility-test', manualOrder: 120, packageScript: 'guard:accessibility-test', requiredScripts: ['config:accessibilityTest.script'], inspectSetup: setup.accessibility }),
  defineNumericCapability({ id: 'quality.architecture', configKey: 'architecture', featureName: 'architecture', featureOrder: 90, doctorOrder: 20, environments: ['manual', 'pre-push', 'ci-full'], defaultTimeoutMs: DEFAULT_ARCHITECTURE_CONFIG.timeoutMs, manualCommand: 'architecture', manualOrder: 40, packageScript: 'guard:architecture', requiredTools: ['dependency-cruiser'], inspectSetup: setup.architecture }),
  defineNumericCapability({ id: 'quality.build', configKey: 'build', featureName: 'build', featureOrder: 110, doctorOrder: 10, environments: ['manual', 'pre-push', 'ci-full'], defaultTimeoutMs: DEFAULT_BUILD_CONFIG.timeoutMs, manualCommand: 'build', manualOrder: 30, packageScript: 'guard:build', requiredScripts: ['config:build.script'], artifactTypes: ['build-output'], inspectSetup: setup.build }),
  defineNumericCapability({ id: 'quality.lighthouse', configKey: 'lighthouse', featureName: 'lighthouse', featureOrder: 120, doctorOrder: 30, environments: ['manual', 'pre-push'], defaultTimeoutMs: DEFAULT_LIGHTHOUSE_CONFIG.timeoutMs, manualCommand: 'lighthouse', manualOptions: ['--skip-build'], manualOrder: 160, packageScript: 'guard:lighthouse', requiredTools: ['@lhci/cli'], artifactTypes: ['lighthouse-report'], inspectSetup: setup.lighthouse }),
  defineNumericCapability({ id: 'quality.style-complexity', configKey: 'preCommit.stylelint.complexity', featureName: 'styleComplexity', featureOrder: 60, environments: ['manual'], manualCommand: 'style-complexity', manualOrder: 130, packageScript: 'guard:style-complexity', requiredTools: ['stylelint'] }),
  defineNumericCapability({ id: 'quality.style-governance', configKey: 'preCommit.stylelint.governance', featureName: 'styleGovernance', featureOrder: 70, environments: ['manual'], manualCommand: 'style-governance', manualOrder: 140, packageScript: 'guard:style-governance', requiredTools: ['stylelint'] }),
]);
