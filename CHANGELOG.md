...
- 修复 GitLab 新分支首次推送的范围基准：获取默认分支并验证唯一共同祖先；不放行缺失或不完整历史。
- 仓库测试默认限制为 4 个文件并发；Windows 外部门禁超时测试核对保留的超时原因及父子进程实际退出，继续拒绝遗留写入。
- 蓝绿备份改为可选：默认只切换应用并保留当前数据；显式启用 `blueGreen.backup.enabled` 后执行快照与恢复。普通模式在正式入口健康检查通过后才停止旧应用，失败回退继续校验健康并准确通知。
- 增加 Java Maven 原生构建与 Docker 蓝绿部署；GitLab 和手动方式共用命令。
- 增加 MySQL、上传目录、独立 Redis 的发布前恢复点、维护切流、健康检查、整套回滚与中断恢复。
- 运维配置独立维护；发布绑定本轮检查、源码提交、构建产物及镜像，失败复用公共退出码。

## 2.0.0 CI 配置执行收口（未发布）

- GitLab 质量模板启用 Bash 子 Shell 执行及逐命令退出码检查，避免旧 Runner 将违规和范围错误统一显示为作业码 1；升级后需重新生成模板。
- 取消公开 CI 档位和观察/强制模式，按项目配置执行并保留不可关闭的公共检查。
- 普通 CI 验证目标提交与工作区/索引一致，本地诊断与 GitLab 接入诊断分离。
- CI 独立支持企业微信与飞书成功/失败通知，仓库配置凭据，默认启用，提供真实接入测试。
- GitLab 按明确分支触发，默认 dev/main，合并请求去重；最终交付改用 delivery-check 入口。

## 2.0.0（未发布）

本轮所有功能统一归入 2.0 重构。此前工作分支及审查文件中的 2.1.0～7.2.0 为未发布的开发阶段编号，不是独立发布版本。各小节汇总开发过程，当前启用状态以最终预设和功能文档为准。

### 目录职责与路径绑定

- 新增应用级目录职责与路径绑定。Java、Node、前端分别提供可编辑目录约定；检查按绑定解析目录引用，用户显式路径优先。同步 AI 规范，严格拒绝循环、越界、未知职责与无效绑定。
- 测试、覆盖率、变异、归位、规模、样式、Java 源码根与架构路径可联动；重复启用不会恢复旧路径。职责不代替业务语义验证，不移动文件或修改原生工具配置。

### Java 默认检查

- Java Maven 新建模板默认开启全部 18 项专属检查，保留工具、模块和插件版本等真实接入缺项；补齐前严格阻断初始化和执行，不安装 Hook。既有配置、阈值和 Node 预设不变。

### Node 默认检查

- Node 新建预设默认开启构建、单元测试、覆盖率、变异测试、架构、Knip、路径命名、函数文档与文件头，TypeScript 预设默认开启类型检查。已有配置保留，普通 Stylelint、图片和代码位置仍关闭。

### 前后端适用性审查

- Prettier 工具就绪按项目 pattern 加载配置，修复 YAML 等目标被额外扩展名过滤的问题。
- 前端与 Node 后端适用性审查：补齐非内联 ESLint 预设工具要求、mts/cts 配置加载目标、单元测试脚本就绪检查；修正后端普通 Stylelint 初始化和 Windows 短路径判定。新增独立 Node 消费项目真实工具验证。

### 前端默认开关

- 修正前端 TypeScript 新建预设遗漏的类型检查开关及工具选项；前端 Stylelint 的 UI Tokens 主开关默认开启，既有显式配置保持不变。同步默认状态文档。

### 提交规范

- 新建配置默认开启提交规范，禁止 merge commit，普通应用不强制提升 major；显式启用补齐预设并保留用户值。保留本地自动文件摘要，受控挑拣只登记为交付合同后续工作。

### 无效代码检查

- 前端新建及显式启用默认开启 Knip 核心检查，新增可修改的内联 options，用户原生配置优先；特殊依赖引用按所属应用跳过。
- 使用实际分析文件计数拒绝空分析通过，保留配置范围、动态引用和外部调用边界。

### 依赖与包管理器

- 修复开发依赖漏洞：Sharp 升级至 0.35.4，colord、fast-uri、js-yaml 的锁定版本更新至已修复范围。

- 支持 npm、pnpm、Yarn 普通直接依赖与锁文件核对，新增明确包管理器、安装根、冲突锁文件和工具就绪配置。
- 删除 allowedProtocols；特殊引用跳过检查，区分声明固定、锁记录一致和真实安装。
- 支持 Yarn PnP 项目解析，托管工程 CI 使用对应冻结安装命令，补充真实联调。

### 移除模板无障碍门禁

- 删除表单标签与图片替代文本门禁、公开命令、固定 Hook/CI 步骤及托管 AI 规范，不提供兼容入口。
- 删除仅供这两项使用的 Vue 策略门禁适配器，保留源码安全、异步资源清理所需的共享解析器和依赖。
- 消费项目需移除旧命令脚本与 CI 门禁配置；详见[删除说明](docs/features/template-accessibility-removal.md)。

### 源码安全

- 新增可编辑的六组前端源码安全配置、统一命令与 Hook/CI 门禁；删除旧命令、旧 Gate ID 和转发分工，统一入口复用精确例外。
- 使用明确的 JS/TS/Vue/HTML/JSX 语法检查，不推导动态值，无法确认和空范围不作为完整通过证据。
- 新增真实 CLI、Schema、误报/漏检/绕过反例测试及源码安全审查记录。

### Token 指定值校验

- 补充消费项目预设设计原则；审查修复函数哈希标识误判、选择器注释误报及 CSS 属性大小写导致的漏检与配置冲突。

- 新增 `checks.stylelint.uiTokens.values`：按清单身份、来源、语言、选择器和条件核对指定值，拒绝缺失、错值及未登记覆盖。
- 新增可选 `uiTokens.artifacts`：真实清理与构建后按显式映射验证 CSS，构建证据绑定 Token 配置和来源实际指纹；不承诺浏览器行为。
- 新增运行时依赖 `postcss-value-parser@4.2.0`，保留消费项目自己的 Stylelint 安装和配置。

### 统一 Stylelint

- 破坏性变更：样式复杂度、治理与 Token 统一至 `checks.stylelint`，移除旧顶层配置、独立功能名和命令；旧配置拒绝而不自动转换。
- 原生规则以用户配置优先，治理改用真实 Vue 与选择器 AST；支持 CSS Modules 全局逃逸检查，修复字符串误判边界。
- 新前端预设开启普通样式与治理；Token 最终预设默认开启，主开关停用保留子配置。统一手动检查保留各项发现并按公共优先级汇总。

### 图片治理

- 审查修复图片审计误放行、动态路径误判、例外与增量范围、设备视口同步、Sharp 依赖检查及批量回滚；Lighthouse 增加每次执行独立诊断，重跑不覆盖失败记录。

- 前端图片全量治理预设，支持用途预算、动画、元数据与可选 AVIF 收益分析。
- 动态图片声明记录接口、响应字段和使用位置，报告保留依据；增加本地失效引用及大小写检查。
- Lighthouse 页面图片审计、路由预算与 DOM 观察；显式批量优化及静态引用更新支持失败恢复。
- 同步 Schema、回归测试及 Skill 接入待办，不新增自动接入 Skill。

### 构建与性能

- 前端显式初始化/启用保存构建预算、Vite 生产选项、包体积分析和 Lighthouse 默认预设，保留已有配置与原生用户覆盖。
- 提供前端工具依赖要求与 Skill 接入待办清单；自动接入 Skill 暂不实现，项目依赖、脚本、目录、Chrome 和业务页面由接入者配置。
- 包体积报告绑定同次构建，构建指纹限制跨阶段复用；Lighthouse 进入完整 CI，验证真实页面和完整报告，取消与超时复用公共进程树处理。


### 公共方法测试

- 删除组件交互和 axe 无障碍测试配置、命令、门禁及六项可选 peer 依赖声明；旧配置键按未知属性拒绝。
- 前端新配置默认开启公共方法单元测试、覆盖率与变异测试，测试集中到 src/tests/utils，保留项目自定义配置。
- 变异测试进入完整 CI 与发布就绪，主配置支持公共方法变异预设，与原生 Stryker 配置合并且原生优先。

### 文件组织与维护

- 不提供依赖包内部路径限制，避免按 src/internal 目录名称误报合法导入。

- 前端初始化保存文件归位、kebab-case 命名、分类行数限制、架构分层、异步清理及文件头预设；既有配置不自动升级。
- 增加可配置的公开函数非空文档检查，贯穿暂存、CI 与交付；无效代码最终预设默认开启，代码位置默认关闭。

### 前端工具预设

- 前端 ESLint、Prettier、Stylelint 和类型检查支持在 `repo-guard.config.json` 保存完整可修改预设；首次启用补齐，重复启用保留用户修改，原生配置按工具语义优先。
- 增加 script setup、类型感知规则、kebab-case 类名和属性排序；类型检查读取项目继承及引用，以临时配置执行只读检查。
- 类型感知暂存检查使用索引快照隔离未暂存源码，保留既有修复顺序和失败恢复。
- 新增 `tool-config` 最终配置查询与前端工具依赖要求接口，同步 Schema、功能说明和真实工具测试。自动安装 Skill 不属于本次交付。

### 工程模型与执行基础

- 修复子应用仅覆盖单个 CI Gate 时丢失仓库默认模式的问题，保留 report、enforce、off 等共享策略，应用显式默认模式仍可覆盖，非法配置继续拒绝。补齐真实 CI 启用与阻断回归。
- 架构图补充已经实现的独立交付、跨仓证据交换、联合验证与人工验收；明确后续扩展指跨仓自动协调发布，避免将已实现的证据协作误写为待实现。
- 补充 Java 现有命令接入说明，明确通用硬性检查、项目依赖版本约束和测试执行证据的边界；Java 原生检查及报告解析仍未实现。

- 修复工作区提升安装且未公开清单的工具误报，统一最近安装、作用域包与目录链接解析；保留符号链接时仍校验入口真实归属，损坏安装不再被其他版本掩盖。
- 流式执行与外部 npm 门禁共用有时限的进程树清理，清理失败返回可定位的执行错误并释放管道，避免超时后继续无限等待；统一启动错误的中文说明。
- Git 错误使用中文主说明，第三方原文进入统一诊断字段并单独展示；Stylelint 样式解析失败同步使用标准诊断，保留配置错误分类。配置快照先确认存在再读取固定对象，索引损坏、对象缺失与真正删除分别处理。
- `full` 复用 `policy` 公共步骤，`release-ready` 继续复用 `full`，消除重复计划定义；同步回归测试、Schema 与功能说明。

- 将状态映射、退出码、进程异常归类及失败优先级收口到 `core/result/exit-code.js`，手动命令、Hook、CI、多应用与独立交付复用统一处理。保留 `0/1/2/3` 码表，移除 Hook 压缩失败类型和多应用取首个非零结果的逻辑。
- 交付检查区分规则/证据未满足和工具异常，`verify`、`accept` 对未满足验收条件统一返回 `2`；查询成功及 CI 只报告保留非阻断语义。新增跨入口回归、AST 出口边界检查及 AGENTS 维护约束，禁止后续入口自行定义码表或透传第三方退出码。

- 样式 Token 检查新增原生 CSS、Less，与 SCSS/Sass 及 Vue 对应内联样式统一使用 `checks.uiTokens.languages`，默认选择 CSS、功能仍默认关闭。移除 UnoCSS 类名/配置分析及旧 `adapters`、`icon` 配置；清单只保留 CSS、Sass、Less 别名，旧字段直接拒绝，不自动转换。
- 保留 12 类设计约束、来源指纹、只读复查和应用级保护；CSS 普通声明使用无回退的完整 `var(--name)`，断点使用团队登记的正长度。受控组件不得重定义已登记变量；同步配置字段说明、AI 规范和回归测试。

当前为开发分支记录，内部重构与回归结果见[重构工作清单](docs/refactor-2.0-remaining-work.md)；此记录不代表 npm 发布已执行。

- 分离公共流程与应用工程策略：前后端各自配置保护规则、例外、依赖、文件归位、CI Gate 策略和外部门禁，根配置管理提交信息、公共基础文件、CI 流程、通知与动画。应用规则不从另一应用或根公共策略中隐式继承。
- 提交、推送及普通 CI 按变更选择受影响应用，新增 `sharedPaths` 显式声明共享文件影响范围；指定应用时延迟加载其工程配置与工具，`release-ready` 默认复核全部应用。聚合报告新增保留应用来源的 `scopedGateResults`。
- 新增独立 `repo-guard.delivery.json` 与共同合同 Schema，支持同仓不同目录及分仓前后端共同交付；可仅使用合同，也可与工程检查组合，Java、Python 参与者无需声明 Node 工程预设。仓库内多文件合同包仍保留当前格式，两种交付入口不能同时启用。
- 新增 `delivery` 初始化、绑定、启停、检查、执行、导入、联合验证和验收入口。合同固定修订与指纹，证据绑定参与方、仓库及代码提交并由受信执行者签名；必需检查关闭或跳过不能算完成，联合命令必须提供本轮确切版本组合的观察报告，人工验收绑定完整证据。
- 新增真实失败反馈与反向改进：实现缺陷保留签名失败结果，修订后要求同一检查定义和测试内容在修复提交上通过，并完成改进检查。同步五个交付 Skills 的独立模式与人工验收密钥边界。

- 修复多应用目录使用 `./`、重复或尾部分隔符时提交策略漏检：配置加载后统一运行相对路径，保留配置原文；新增真实暂存图片命名、跨应用重命名、根目录应用及磁盘/快照读取回归。

- 统一仅使用当前格式：功能登记表、托管 Skill 清单、UI Token 清单、无效代码/构建产物基线、CI 报告与外部门禁报告升级为 v2，外部门禁标识改为 `repo-guard-json-v2`，旧输入直接拒绝。合同与 GateResult 保持已实施的 v2。
- 移除 Hook v1～v4 标记和 AGENTS 旧区块转换，仅接受当前 Hook v5 与当前职责区块。旧/未知文件不覆盖；执行锁升级 v2，旧版本锁阻断而非自动删除。同步仓库规范、Skill 资产及只读拒绝回归。

- 统一原生 v2 执行模型，配置校验、默认值、功能开关、Gate 声明、Doctor、Hook、CI 和 AGENTS 使用相同分区；删除 v2 转回 v1 的运行适配与 `configVersion` 双版本标记。
- 删除旧项目配置解析器、冻结默认值、字段转换、迁移 CLI/API 与配套回滚模块。项目配置仅接受 `version: 2`，非 v2 输入直接拒绝且原文件不变；质量配置不接受旧 `ci.pipeline`。
- 消费项目 `release-ready` 统一使用完整工程检查、Lighthouse、外部门禁和交付证据计划，移除 `release.check / release.test / release.package`；npm 包自身发布验证仅由维护者发布 Skill 管理。
- GitLab 质量安装器只生成质量任务；独立运维配置管理通知，删除旧发布渲染器、薄桥与旧模板转换器；只更新当前可验证的托管内容，旧模板、无摘要片段及人工修改均拒绝覆盖。
- 修复 CLI 重装质量 CI 时隐式降级配置档的问题：省略 `--profile` 沿用已有配置，只有显式传入才覆盖，并验证配置与生成 YAML 保持一致。
- 修复独立运维成功、失败通知未加入 MR 流水线的问题：通知与质量、构建复用相同匹配规则，保留状态条件与去重逻辑；同步单应用及工作区 Schema 的质量 CI、工程交付检查说明，并增加防回退测试。
- 常规测试直接构造并断言 v2 模型，旧项目配置仅用于拒绝输入的反例；增加转换入口不存在和拒绝时原字节不变的回归，继续覆盖暂存恢复、配置快照、跨应用路径归属和 CI 失败汇总。

- 重构公开配置为 v2：由人或 AI 明确声明项目身份、前后端角色与技术栈预设，按 `checks`、`repository`、`reporting` 和 `ci` 分离应用检查与公共规范。旧配置不再兼容，由人工保存原资料并按新架构重新建立；运维使用独立文件。
- 新增 Node JavaScript / TypeScript 后端预设，复用代码规范、依赖、类型、架构、测试、覆盖率、变异测试和构建能力；按项目身份排除 Vue 与页面专用检查，不新增业务规则或接口契约校验。
- 支持显式多应用工作区与 `--project` 选择，工具和路径按应用解析。仓库 Hook 保持固定修复顺序及暂存隔离，公共策略统一执行，Git 配置快照与多应用 CI 报告分别保留来源和检查范围。
- 提取独立运维模块与 `repo-guard.ops.json`，提供 GitLab 流水线预览和安装，各应用独立绑定质量检查、构建产物及发布环境，生产部署默认人工触发。v2 发布就绪执行通用工程检查，不再要求消费项目具备 npm 包发布专用流程。
- 测试按核心、配置、规范、门禁、Hook、CI、运维等功能分组，增加跨应用 Git、原生配置与拒绝旧输入、隔离及流水线回归。同步配置 Schema、README、使用说明、逐项功能文档及架构图。
- Java Maven 工程检查已完成适配；检查工具自动安装与项目原生配置自动补齐仍为后续工作。

## 1.24.1

- 重写 README，以团队规范自动检查的具体场景解释项目用途，提前展示真实提交检查与首次使用步骤，说明与项目既有工具的配合方式。
- 按规则配置、团队协作和完整交付组织介绍，解释交付合同及人、AI、repo-guard、Git 的分工，将猫狗动画放在核心用途之后。
- 增加使用说明中全部 31 项可配置功能的完整截图，在开头与导航突出能力数量，并保留可复制的功能表和详细文档入口。本版本不改变运行行为和配置契约。

## 1.24.0

- 新增默认关闭的 `commitAnimation`：小猫、小狗终端动画、提交类型道具及内置随机成功彩蛋；真实检查失败保留完整中文报告，仅在 Git 创建提交后的 `post-commit` 庆祝。
- 提交道具覆盖 `feat`、`fix`、`docs`、`style`、`refactor`、`perf`、`test`、`build`、`ci`、`chore` 全部默认类型，与校验共用提交标题语法；特殊提交和自定义未知类型使用普通包裹。成功彩蛋类型与概率内置，仅预览命令可指定彩蛋，不提供公开配置。
- 新增无 Git 写入的 `animation-preview` 命令；CI、非交互输出、无颜色与小窗口自动保留文字报告。托管 Hook 升级至 v5 并兼容已知旧标记，补齐配置 Schema、功能启停、专题文档和暂存隔离回归验证。
- 中断先恢复光标再交还原有信号处理，终端检测与绘制异常自动降级且不遗留动画计时器；提交后入口读取不到 Git 提交记录时不误报成功。
- README 提前介绍提交动画，增加新功能标识、猫狗动图和十种道具图；同步功能手册与 31 项能力开关说明，并在仓库 AGENTS.md 中明确后续工作分支的命名与创建规则。

## 1.23.1

- 为各功能专题的 JSON 配置示例补充相邻字段表，说明用途、可填值、默认值、范围和联动要求；为交付合同 YAML 添加字段注释，并将配置说明同步纳入长期文档维护约定。
- 重写根目录 README，以规则配置、提交前统一检查、完整交付闭环为主线，提前展示安装步骤，增加可复制配置、按用途分类的功能入口和真实提交演示；重新绘制浅色配置、提交检查与交付闭环 SVG，移除旧首页横幅和 HTML 导出页。
- 重组使用说明为接入与日常操作手册，为全部 30 项功能开关补充用途、配置位置、默认状态、执行入口和详情链接；校准 CI 配置档、手动开关、外部门禁环境及退出码说明。补齐 53 份专题文档，覆盖功能索引中的全部 56 项能力入口。
- 将功能登记、合同、交付证据和真实反馈集中到一份交付合同手册，补充人、AI、repo-guard、Git 的四方时序及真实测试反馈反向升级流程，提供 SVG 和可编辑 Mermaid 源文件；明确两轮证据复核、人工验收及稳定结果字段，删除旧格式页并将交付 Skill 引用更新为统一手册。
- 将项目总览收窄为维护者架构说明，保留代码结构、模块职责、依赖边界和工作模型；建立逐项文档维护约定，并校验开关完整性、专题入口、统一交付文档与配图。

- 补充 npm 标准 `homepage`、`repository` 和 `bugs` 元数据，分别指向 GitHub README、源码仓库和问题反馈页。
- 将仓库维护者的版本判断、发布前验证、npm Web 登录、2FA、发布和注册表复核流程从根目录文档迁移为 `.agents/skills/repo-guard-publishing` 标准 Skill；Skill 区分发布准备与真实发布授权，禁止把认证秘密写入仓库或日志，并且不会随 npm 包分发给消费项目。
...
- 使用消费项目安装的 Sharp 和 SVGO 评估无损压缩及 WebP 转换收益，支持体积、字节和百分比三重阈值、像素/帧/输入大小上限及 `report`/`error` 策略；Hook 与 CI 全程只读，不自动删除资源、改写引用或执行有损转换。
- 新增 `repo-guard image-optimize` 显式命令；默认只预览，`--write` 仅处理范围内、由 Git 跟踪且无修改的源图，原格式压缩通过临时文件与备份安全替换，WebP 生成同名兄弟文件并保留原图。JPEG 或有损 PNG 转换需要配置和 `--allow-lossy` 双重确认，SVG 写入也需单独授权。
- 接入 Registry、固定 pre-commit/CI 计划、doctor、项目脚本、配置 Schema、AGENTS 托管规范与结构化例外；图片命名与全项目路径命名同时启用时强制使用同一种规范，并补充真实 Sharp/SVGO、暂存区二进制快照、重复资源、格式错配和防覆盖回归测试。
...
- 在 README 中明确项目用于约束 AI 辅助开发，防止随意修改受保护文件、绕过质量检查、破坏目录与依赖边界或未经验证直接交付。
- 收敛 README 为项目定位、已完成功能和文档导航，将安装、命令、配置及门禁接入细节迁移到独立使用说明，并同步长期项目结构清单。
...
- 强制该 runner 对应的 `externalGates` 只能配置 `environments: ["manual"]`，同时拒绝常见 CI 和流水线环境标记；不注册官方 Gate，不进入 pre-commit、pre-push、固定 CI、发布准备、受保护构建或打包流程。
- 目标环境必须使用 HTTPS、精确主机白名单和每次运行确认值；默认只允许只读请求，写请求要求全局与场景双重授权及强制清理，清理失败不生成可被误用的主报告。
- 新增独立配置 Schema、中文阈值问题项、报告目录忽略/符号链接/已跟踪文件保护，以及通过、违规、执行错误、CLI 路由和自动环境拒绝的回归测试；同步 README 和长期功能清单。

## 1.13.0

- 新增默认关闭的 Stryker 10.x 变异测试 Gate；使用消费项目自身的 Stryker、测试运行器和配置，强制非原地变异与本地 reporter，不隐式上传报告，并要求 Stryker `thresholds.break` 形成硬门槛，缺失时也会阻断。
- 生成 UTF-8 中文 HTML 报告，同时保留经过校验的 Stryker JSON 和可选原始 HTML；每次执行前删除旧报告，拒绝旧文件、未知状态、无效阈值、路径穿越、符号链接和已跟踪文件覆盖。
- 新增可配置的多构建脚本保护：`guard:build:*` 先运行变异测试，通过后才执行对应原始 npm 构建脚本；失败时可复用现有企业微信通知，原始构建脚本保持不变。
- 接入 Registry、手动命令、doctor、`repo-guard init` 脚本同步、配置 Schema、README、长期功能清单和端到端回归测试；变异测试不进入 pre-commit、pre-push 或固定 CI 计划。

## 1.12.1

- 中文用户文案检查从仅识别纯英文扩展为同时识别中英文混合说明，阻止通过加入少量汉字绕过零英文债务契约；命令、路径、包名、规则 ID、配置字段和受控状态枚举继续保持稳定原值。
- 修复本地通知配置、pre-push 快照、TypeScript、ESLint 预设、单元测试、无障碍测试、依赖锁文件、发布元数据和 GitLab CI 设置中的中英文混合用户提示，并保持第三方原始错误仅作为受控诊断内容传递。
- 补充纯英文、中英文短语、单个未翻译说明词、机器标识和命令的回归测试；迁移基线继续保持 `0/0`，不新增或替换任何允许项。
...
- 不可变文件违规现在输出中文 Git 状态、原路径与目标路径、期望状态和恢复步骤；发生硬阻断时不会先执行企业微信通知等外部写入。
- 保持 `audit`、`notify` 和 `ci.protectedFiles.action: "fail"` 原有语义，并同步配置验证、Schema、README 与回归测试。
...
- 将中文文案检查接入 `npm run check`，以精确指纹完成 1.4.98 历史英文债务迁移并将基线收缩到 0；新增、替换或复制纯英文用户文案都会失败，且检查入口不得提供扩大或重新生成基线的绕过能力。
- console renderer 统一使用“通过、跳过、未通过、错误、证据、修复目标、处理方式、需要人工确认”等中文标签；问题 fingerprint 改由稳定机器字段和位置生成，不再因英文迁移到中文或措辞调整而漂移。
- 外部门禁报告中的中文摘要、问题、证据、修复说明和产物说明保持原样；非中文内容不会提升为主要问题文案，而是由 repo-guard 提供中文回退说明，并将经过脱敏和截断处理的第三方原文保留在明确标记的 diagnostics 中。
...
移除 UI Tokens 的 iconSelectors、icon-size 类别和基于选择器的图标尺寸检查，保留其他分类。旧字段直接拒绝，不做迁移。
