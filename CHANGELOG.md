...
- 重新设计 README 的信息架构：使用经过结构和几何校验的六层 SVG 生命周期能力分层图，按项目接入、需求与开发、`git commit`、`git push`、GitLab CI、测试与发布归纳全部已实现功能和真实反馈闭环；不再保留与分层图重复的独立流程图，并删除重复的功能长清单与文档、项目链接章节，完整操作细节继续下沉到使用说明和长期功能清单。
- 补充 npm 标准 `homepage`、`repository` 和 `bugs` 元数据，分别指向 GitHub README、源码仓库和问题反馈页；这些链接会在发布本版本后进入 npm 注册表元数据。
...
- 将长期项目文档从功能细节清单调整为项目结构与能力总览：删除版本分支递进树，新增工作模型 SVG，集中说明可信事实、统一判断、生命周期编排、输出契约和模块职责；新增 `docs/features/README.md`，为单项功能说明建立可核对的已维护/待编写索引。
