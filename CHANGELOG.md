...
当前为开发分支的阶段性记录，内部配置统一及旧 CI 发布链路清理尚未完成，详见[重构剩余工作清单](docs/refactor-2.0-remaining-work.md)。

- 重构公开配置为 v2：由人或 AI 明确声明项目身份、前后端角色与技术栈预设，按 `checks`、`repository`、`reporting` 和 `ci` 分离应用检查与公共规范。旧配置需显式迁移并保留原始备份；旧运维配置需先明确拆分。
...
- 测试按核心、配置、规范、门禁、Hook、CI、运维等功能分组，增加跨应用 Git、配置迁移、隔离与流水线回归。同步配置 Schema、README、使用说明、逐项功能文档及架构图。
